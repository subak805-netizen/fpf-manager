...
function buildMessage(brief, now) {
  ...
  const inspG = (title, act, arr) => { // 검사류: 공장이 전부 같으면 헤더에 1번만
    ...
    const fcs = [...new Set(arr.map(x => x.fc).filter(Boolean))];
    const one = fcs.length === 1;
    return G(title, act, arr.map(x => x.row + (!one && x.fc ? ' (' + x.fc + ')' : '')), one ? fcs[0] : '');
  };

  // --- 급한 것 (종류별 묶음) ---
  const uG = [];
  uG.push(G('출고 지연', '공장에 진행 확인 전화',
    (brief.lateShip || []).map(x => nm(x.o) + ' (예정 ' + mmdd(x.d) + ', ' + Math.abs(dayDiff(x.d, today)) + '일째)')));
  ...
  (brief.alerts || []).forEach(a => {
    ...
    const row = ((a.o ? nm(a.o) + ' / ' : '') + vendor).trim() || (a.d || '');
  ...
  // --- 오늘 (종류별 묶음) ---
  const tG = [];
  tG.push(inspG('검사', '작지·검사서류 챙기기', (brief.insp || []).filter(x => x.d === today).map(x => ({ row: nm(x.o), fc: x.fc }))));
  tG.push(G('출고', '출고 전 장수 확인 문자 받기', (brief.ship || []).filter(x => x.d === today).map(x => nm(x.o))));
  tG.push(G('입고', '도착 확인되면 공장에 재단 투입', (brief.arrive || []).filter(x => x.d === today).map(x => (x.sup ? x.sup + ' / ' : '') + nm(x.o))));
  tG.push(G('픽업', '', (brief.pickups || []).filter(p => p.w === '오늘').map(p => [p.sup, p.ty, p.it].filter(Boolean).join(' / '))));
  ...
  // --- 미리 준비 (1~3일 내, 묶음) ---
  const pG = [];
  pG.push(inspG('검사 예정', '공장에 진행 상황 확인 연락', (brief.insp || []).filter(x => { const d = dayDiff(x.d, today); return d >= 1 && d <= 3; }).map(x => ({ row: 'D-' + dayDiff(x.d, today) + ' ' + nm(x.o), fc: x.fc }))));
  pG.push(G('출고 예정', '장수·일정 미리 확인', (brief.ship || []).filter(x => { const d = dayDiff(x.d, today); return d >= 1 && d <= 3; }).map(x => 'D-' + dayDiff(x.d, today) + ' ' + nm(x.o))));
