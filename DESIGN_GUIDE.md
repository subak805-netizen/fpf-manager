...
13. **★작업지시서 테두리 = 2px 위계 정식 채택 (2026-06-09 사용자 "목업과 완전 동일" 요청 → 점3 예외).** 뼈대 굵은선 **2px `#6b7280`**(외곽 `.tp-wrap`, 헤더↔본문 `.htop` 1.5px, 도식화↔데이터 `.colL` border-right, 각 섹션 `.sec` border-top, `.topnote` border-bottom), 내부 셀 **1px `#d1d5db`**(연한 회색), 이미지 드롭존 **1px dashed `#9ca3af`**. 셀 배경: 헤더 `#f3f4f6`, 요척 `.yc #2563eb`, **컬러웨이 그룹 배경 = 진한 파스텔 팔레트**(`#f3d0d6`핑크·`#d9ecc9`연두·`#d3e3f5`블루·`#f6e8c8`크림…). 폰트 IBM Plex Sans KR, th 700. → 목업 `mockups/techpack-fruto-landscape.html`과 동일. (점3의 "1px 전용"은 작업지시서에 한해 무효.)
