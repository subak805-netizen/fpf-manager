...
## 2026-09-04a — 아이패드가 옛 자료로 뜨고 맥북을 덮으려던 것 (동기화 4건)
8월 치료 뒤 «라이브 폰↔PC 미검증» 이었던 부분에서 나온 증상. 뿌리 4개:
1. **옛 버전 앱을 막지 않았다** — `fpfCheckAppVersion` 이 부팅 몇 초 뒤 띠만 띄우고 옛 코드가 계속 돌았다.
   옛 코드 = 옛 병합 규칙 = 옛 자료를 그대로 밀어올림. → `onFirebaseLogin` 맨 앞에서 **먼저** 확인하고,
   옛 버전이면 `_appStaleBlock` 로 푸시를 영구히 막고 자동 새로고침(세션당 1회 `fpm_autoReload`, 루프 방지).
   ⚠️ 판정은 «다르면»이 아니라 **«서버가 더 새로우면»**(`String(latest)>String(APP_BUILD)`) — 배포 직후 CDN 이
   옛 파일을 주면 내가 더 새로운데도 stale 로 오인해 새로고침 루프에 빠진다.
2. **20초 안전장치가 푸시를 풀었다** — 느린 망에서 받는 중인데 20초가 지나면 푸시 허용 → 옛 자료 업로드.
   → 60초로, 풀리더라도 `_forceStaleMerge` 를 켜서 `cloudSyncSave` 가 **먼저 `cloudSyncLoad` 로 받은 뒤에만** 올린다.
   못 받으면 안 올린다(로컬엔 남음). 정상 로드가 끝나면 플래그 해제.
3. **화면 복귀 때 밀어올리기만 하고 안 받았다** — 아이패드·폰은 뒤로 가 있는 동안 실시간 연결이 끊기고,
   `startRealtimeSync` 를 다시 걸어도 첫 스냅샷은 «이미 가진 것»으로 건너뛴다 → «새로 켤 때만 동기화» 체감.
   → `_hiddenAt` 기록, 60초 넘게 숨어 있었으면 `cloudSyncLoad` → render → `startRealtimeSync`. 편집 중이면 30초 뒤 재시도.
4. **배너가 «뭐가 바뀌는지»도 «거부»도 못 했다** → `_rtPendingSummary()` 가 대기분(아이템 상자·발주 조각)을 내 것과
   비교해 달라진 개수·이름을 요약. 「받기」/「내 것 유지」 두 버튼. 「내 것 유지」(`rejectPendingRemote`) = 확인 후
   달라진 아이템·오더에만 `_mt=now` 도장 → `_rejectRemoteOnce` 로 그 한 번은 스테일 병합을 건너뛰고 푸시 →
   다른 기기 병합에서 내 것이 이긴다. ⚠️ 상대의 진짜 수정도 지워질 수 있어 confirm 을 받는다.
검증: 실제 함수 하니스 26개(옛버전 판정 4·요약 4·배너 4·거부 8·받기 2) + 돈 검산 69.

## 2026-09-04b — 배너·「받기」·「내 것 유지」 전부 없앰 → 편집 중에도 조용히 합침
- 요청: 「받기, 내 것 유지 이런 게 아예 없었으면」. 맞는 지적 — 배너는 «어느 쪽이 맞는지»가 아니라
  «타이핑하던 칸이 날아갈까 봐» 미뤄둔 것이었다. 판단은 항목별 `_mt`(나중 것이 이김)로 시스템이 이미 한다.
- **고침**: 실시간 리스너 4곳(메인·보조상자·발주조각·업체단가장)에서 `isEditingNow()`면
  `applyRemote*(…,{noRender:true})` 로 **자료만 합치고** `window._rtNeedRender=true`.
  `_rtRenderWhenIdle()` 이 3초마다 + `focusout` 때 «편집 중 아님»이면 한 번 그린다(스크롤 유지·토스트).
  `applyRemoteOrders`·`applyRemoteShared` 에 `opts.noRender` 추가(예전엔 항상 그렸다).
  스테일 저장 경로(`cloudSyncSave`)도 편집 중이면 `_rtNeedRender` 예약.
- `showRemoteBanner` 는 빈 껍데기(호출부가 남아 있어 삭제 대신). `applyPendingRemote/rejectPendingRemote/_rtPendingSummary` 는
  더 이상 도달 안 함 — 대기분(`_rtPending*`)이 채워지지 않는다.
- ⚠️ 편집 중인 바로 그 아이템을 상대가 고쳤으면, 내가 저장할 때 내 `_mt` 가 새로워 **내 것이 이긴다**(폼이 열려 있는 동안은
  폼 메모리가 원본). 예전 「받기」를 눌러도 같았다. 이건 의도된 동작.
- 검증: 실제 함수 하니스 16개(배너 없음·편집중 미룸·폼/작지 열림 미룸·끝나면 1회 그림·noRender 가드 4).

## 2026-09-04c — 세로 스와치: 컬러명 칸 좁게, 배색 칸 넓게
- `table.tp-swv th.tp-swvh` 104→**80px**, 긴 이름(네이비/올리브)은 `white-space:normal` 로 두 줄(잘리지 않음).
- 세로 배치에서만 몸판:배색 = **1.15:1** (`table.tp-swv .tp-swbox.body{flex:1.15}`, `.tp-swsub{flex:1}`).
  가로 배치의 2:1(`#tp-modal .tp-wrap .tp-swbox.body{flex:2}`)은 그대로 — 선택자에 `table.tp-swv` 를 넣어 특이도로 이김.
- 실측(오른쪽 칼럼 52% 기준): 컬러명 80 · 몸판 159 · 배색 139px.
- 참고(질문 답): 배색 칸은 `_hasSubFab`(원단 카드 2개↑ 또는 용도≠몸판 원단) 또는 이미 올린 배색 사진이 있을 때만.
  단추 칸은 부자재에 `orderType==='button'` 이 있을 때만. **부자재로 등록한 주머니속·안감은 원단 개수에 안 잡힌다.**

## 2026-09-04d — 배색 스와치 칸: 부자재 안감·주머니속도 «감»으로 본다
- 요청: 「융TC는 부자재집 거라 원단으로 못 옮겨 — 안감·주머니속 이런 것도 칸이 생겼으면」.
- 규칙을 `_tpHasSubMat(it)` 로 뗐다. ① 원단 2개↑ 또는 용도≠몸판 원단(예전 그대로)
  ② **부자재라도 용도가** `_TP_SUB_PART_RE`(안감·주머니속·주머니·배색·시보리·에리감·스트링감·안단·덧단)에 걸리면.
  단 `_TP_NOT_MAT_OT`(단추·지퍼·스냅·실·케어라벨·메인라벨)은 용도가 뭐든 제외 — 감이 아니다. 심지(용도 접착)는 정규식에 안 걸려 제외.
- 검증 13개: 예전 규칙 4 유지 · 새 규칙 4(세무팬츠 케이스 포함) · 제외 4 · 빈값 1.
- ⚠️ 용도 낱말을 늘릴 땐 `_TP_SUB_PART_RE` 만 고치면 된다. 부자재 유형을 새로 만들면 «감이 아닌 것»이면 `_TP_NOT_MAT_OT` 에 넣을 것.

## 2026-09-04e — 오무데를 드롭다운(우상/좌상)에서 «직접 적는 칸»으로
- 요청: 「우상·좌상 말고도 추가할 수 있게 — 기모난면이 오무데 이런 식으로」.
- 네 군데 전부 `<input list="dl-omude">` 로: 원단 카드 · 부자재 카드(`omudeTrimFld`) · 단가장 상세(`_pbdOmude`) · 단가장 원단 표 셀.
  값은 원래부터 문자열이라 저장(`saveToPB`)·수집(`colFR/colTR`)·자동채움(`autofillFab/Trim`)·잠금(`_lockTrimPbFields`)·작지 표시는 손 안 댐.
- 추천 목록 `buildOmudeDL()`(onfocus): 우상·좌상 + 단가장·아이템·폼에 이미 적어둔 문구 전부(공백 정리·중복 제거·빈값 제외).
  한 번 적은 「기모난면이 오무데」는 다음부터 추천에 뜬다.
- 잠금: select 때는 `tcd-ro`+tabindex, input 이 되면서 `readOnly` 경로 — `_lockTrimPbFields` 가 이미 둘 다 처리한다(검증됨).
- 검증 12개: input 전환·자유 문구 유지·추천 5종·중복/빈값/공백·등록 부자재 잠금+단가장값 채움·미등록 유지.

## 2026-09-04f — 단가장 「새 품목 2·3·4·5」 증식 + 단위 드롭다운
### ① 새 품목이 자꾸 생김
- **뿌리**: `renamePBMaterial` 이 옛 키를 `delete` 만 하고 **묘비를 안 남겼다**(8월 감사 「묘비 누락 삭제」 목록에 있던 것).
  → 「새 품목」을 만들고 이름을 바꾸면, 클라우드 합집합이 옛 키 「새 품목」을 되살리고, 다음 「+품목」은 새 품목 2·3·4·5 로 밀렸다.
- **고침**: 키가 바뀌면 옛 키에 `_tombArr('pbMat', 거래처\u001f옛키)`, 새 키엔 `_untombArr(…)`(예전에 지운 이름을 다시 쓸 때).
- ⚠️ 단가장 품목의 «키 변경 = 삭제+추가» 인 곳(`renamePbSupplier`·`pbMergeGroup` 등)도 같은 규칙이어야 한다 — 아직 안 봤다.
### ② 단위: Y · R · 콘 · ea · set · 묶음
- 예전 드롭다운은 계산용 `orderType`(ea/Y/롤/실/지퍼/단추)을 직접 바꿨다. 이제 **«단가가 무엇당인가»(`m.unit`)** 를 적는다.
- `updatePBUnit`: Y→yard · R→roll · 콘→thread 는 계산 유형도 같이 맞추고 아이템에 전파. ea·set·묶음은 개수 계산이되
  **단추·지퍼·스냅·라벨·바이어스(`_PB_OT_KEEP`)는 유형을 건드리지 않는다**(디테일 칸이 사라지면 안 되니).
- 보여줄 단위 `_pbUnitOf(m)`: `m.unit` 이 목록에 있으면 그것, 아니면 계산 유형에서 유추(yard→Y·roll→R·thread→콘·나머지 ea). 옛 'y'/'yd'도 Y 로.
- 원단 단가장 표의 유형 셀은 여전히 `updatePBOrderType`(YARD/KG 배지) — 손 안 댐.
- 검증 18개: 묘비·합집합 필터·다음 +품목이 다시 「새 품목」·두 번째 이름 변경 / 목록·유추 8종·Y→R 전파·콘·묶음·단추 유형 보존.

## 2026-09-04g — 「새 품목 다 지웠는데 또 생겨」(같은 날 되살림 동률) + 삭제·쓰는 곳 맨 왼쪽
### ① 지워도 되살아나던 뿌리 — 내가 02q 에 넣은 되살림 규칙의 구멍
- 묘비(`pbMat`)와 되살림(`un:pbMat`)이 **날짜(일)** 값이라, 같은 날 «만들기(되살림 09-04) → 지우기(묘비 09-04)» 를 하면
  날짜가 같아 `u>=t` 로 **되살림이 이겼다**. 로컬에선 `_tombArr` 가 un: 을 거두지만, 클라우드 합집합(`_mergeTomb`)이
  un: 을 되가져오면 삭제가 무효 → 지운 「새 품목 3·4」가 다시 나타났다.
- **고침**: `_tombStamp()` = ISO 시각(초·ms까지). 묘비·되살림 둘 다. `_tombHas` 는 **되살림이 «더 나중»일 때만** 무시(`u>t`, 동률=삭제 승).
  ISO 는 사전순=시간순이라 옛 날짜값('2026-09-01')과도 그대로 비교되고, 90일 청소(`_cut` 날짜 문자열 비교)도 그대로 돈다.
- 검증(실제 함수): 만들기→3ms 뒤 지우기→합집합이 un: 되가져옴→**여전히 지워진 채** / 지운 뒤 더 나중에 다시 만들면 살아남 / 옛 날짜 묘비 호환.
- ⚠️ 앞으로 «나중 것이 이긴다» 류 비교는 **날짜가 아니라 시각**으로. 날짜 동률은 반드시 생긴다.
...
## 2026-09-04i — 작지 코멘트 패널에 회차 삭제
- 카드 머리 「확인」 옆 **「삭제」**(`.tpc-del`). `tpCmtDel(ri)` = `scDelRound` 와 같은 자료 처리(확인창→splice→`_scWrite`)지만
  **`renderSampleComment()` 를 부르지 않는다**(코멘트 탭의 `window._scOpen` 을 바꿔버림 — 패널 규칙과 동일).
- 확인창에 글·사진 개수를 알려주고, **작지 샘플·패턴 회차는 지우지 않는다**고 적는다(치수·도식화가 들어 있어 따로).
- 검증 15개: 취소 유지 · 안내문 · 해당 회차만 삭제 · 재렌더·배지 · `_scOpen` 불변 · 전부 지우면 빈 안내 · 없는 인덱스 무시.

## 2026-09-04j — 안사양 지시서 탭 · 완성 세로판 이미지 1칸 · 송장 직접 입력(한 번만)
### ① 안사양 지시서 (탭 `inn`, 지시서 바로 뒤)
- 요청: 「지시서 뒤에 안사양지시서 — 다 똑같은데 이미지만 다른 걸 넣을 거야」.
- 지시사항·견본·워싱·실컬러·스와치·원부자재·치수·라벨 = **지시서와 같은 자료(sew.*)** 를 비춘다(한쪽에서 고치면 양쪽). 도식화 사진만 따로 `images.innSketch`.
- 모양: 지시서 CSS 선택자 14개를 `.panel[data-panel="sew"]` → **`.panel.sewlike`** 로 바꾸고, sew·inn 패널 둘 다 `class="panel sewlike"`.
  ⚠️ 앞으로 지시서 모양을 고칠 땐 `.sewlike` 로 — `[data-panel="sew"]` 로 쓰면 안사양엔 안 먹는다.
- `PROC.inn='안사양'`(제목 「작업지시서(안사양)」), `_validTabs`·탭 순서·패널 조립(`sew+inn+spat…`)·`_tpSmpPics` 에 innSketch 추가.
### ② 완성 작지 세로판 — 위 3칸(앞·뒤·디테일①)을 한 칸으로
- 세로(`isPort`)면 `.tp-pack-imgs.one`(1열) + 「앞면 / 뒷면」 큰 칸 + 「디테일 ②」. 가로판은 2×2 그대로.
- ⚠️ 옛 `packBack`·`packDetail1` 에 **사진이 있으면 그 칸을 그대로 보여준다**(숨겨서 사진이 사라진 듯 보이지 않게).
### ③ 경기택배 송장 — 「직접 입력 (한 번만 보낼 곳)」
- 요청: 「저장한 거 불러오는 거 말고도 새로 적어서 한 번만 뽑을 수 있게」.
- 카드 목록 맨 위 섹션 「직접 입력」 + 「＋ 새로 적어서 한 번만 보내기」. 칸 = 수신·담당·전화·주소 + 운임/비고/브랜드(등록 카드와 공용 `_ciCardBodyHTML`).
- 자료는 `window._ciB.adhoc` 에만(저장 안 됨·새로고침에 사라짐 = 목적). 시재 기록은 이름으로 똑같이 남는다.
- `_ciAll()` = 등록(`_ciFacs`) + 직접(`_ciAdhocList`) — 개수·미리보기·인쇄가 이걸 쓴다. 「전체 선택」은 등록된 곳만.
- 타이핑마다 카드를 다시 그리지 않는다(`ciBAdSet` = 상태·미리보기·체크표시만) — 커서가 날아가서. 이름을 처음 적으면 자동 체크.
- 이름 없는 직접 칸이 체크된 채 인쇄하면 안내(「직접 입력 칸에 수신 이름을 적으세요」)만 하고 멈춘다.
- 검증(실제 함수 17개): 자동체크·개수·미리보기·인쇄 장수·시재 1건/중복없음·빈 이름 안내·삭제·검색에도 남음·전체선택 제외·포커스 유지.

## 2026-09-04k — 스와치 「＋ 칸 추가」 (아이템 공통 추가 스와치 칸, 미리보기 승인)
- 자료: `techpack.swExtra=[{k:'x…',name:'안감'},…]` · 사진 슬롯 `swX_<k>_<컬러키>` (옛 컬러명 폴백 `_swAssign` 그대로).
- `tpSwatchAttachHTML` 이 모든 컬러 줄·모든 시트(지시서·안사양·샘플 회차·그레이딩)에 같이 그린다. 몸판·배색·단추 그대로.
- 칸 = `.tp-swbox.xtra[data-k]` : 위 알약 이름(`tpSwExtraName` — tpSwPart 식 디바운스+전 컬러 동기화) + 왼쪽 위 × (`.tp-swxdel.tp-editonly`, `tpSwExtraDel` 확인창에 사진 장수, 사진도 같이 '' ).
- 표 아래 오른쪽 「＋ 칸 추가」 바(`.tp-swbar.tp-editonly`, `tpSwExtraAdd` = prompt). 인쇄·이미지 저장엔 tp-editonly 로 숨음.
...
## 2026-09-05a — 옛 버전 기기가 «스스로» 최신으로 다시 열림 (아이패드 「아직도 최신으로 동기화 안 돼」)
- 원인 후보: 홈화면 앱을 오래 켜둔 기기(아이패드·맥)는 부팅 게이트(09-04a)를 다시 만나지 않는다. 10분 검사·화면 복귀 검사는 **띠만 띄우고**(「나중에」 가능)
  옛 코드가 옛 규칙으로 계속 동기화했다.
- 고침: `fpfAutoUpdate()` — 10분마다·화면 복귀·부팅 8초 뒤. 옛 버전이면 토스트 후 `fpfHardReload()`. 편집 중이거나 올릴 게 남아 있으면(`_cloudSyncTimer`) 띠만.
  같은 서버 버전엔 sessionStorage(`fpm_autoUp_<latest>`)로 한 번만(루프 방지) → 그 뒤엔 띠.
- `fpfHardReload` 가 «시작 주소(물음표 없는 주소)»를 `fetch(cache:'reload')` 로 새로 받아 둔다 — 홈화면 앱은 다음 실행 때 그 주소로 열리기 때문.
- 검증(실제 함수, jsc): 옛→다시 열기·시작주소 캐시 갱신·두 번 안 함·편집 중 띠만·올릴 것 있으면 띠만·최신이면 무동작·CDN 옛 파일이면 무동작 7개.
- ⚠️ 이미 옛 빌드로 얼어 있는 기기는 새 코드가 없으니 스스로 못 고친다 — 그 기기에서 앱을 완전히 닫고 다시 열거나 「최신으로 새로고침」 한 번.

## 2026-09-07a — 단가장 품목 사라짐(재발) 근본 치료 + 복구 도구 + 단가장 표에 무게·온즈·오무데
### 원인 (신고 「내가 다 등록해서 불러왔는데 지금 또 다 미등록이래」「단가장엔 또 없어 다 사라졌어」)
- 아이템 카드 「단가장 등록」(`pbRegisterFormMat`)이 **되살림 표시(`_untombArr`) 없이** 품목을 넣었다. 예전에 같은 이름을 지운 묘비(90일 보관, 04g ISO 시각)가
  클라우드 합집합으로 내려오면 `_mergePB`/`_mergeSharedSmart` 가 그 품목을 또 지웠다 → 등록해도 동기화 한 번에 사라짐.
- 보조 구멍: `cloudSyncSave` 가 공유문서(업체·단가장) 읽기에 실패하면 **병합 없이 통째로** 올렸다(느린 망의 옛 로컬이 클라우드를 덮음).
### 고침
- `_pbReviveExisting()` — saveData 마다: 지금 로컬에 «있는» 품목·거래처에 살아있는 묘비가 있으면 되살림 표시. 단 품목 `lastUpdated`(날짜)가 묘비 날짜보다
  옛날이면 «다른 기기가 이름 바꿔 지운 옛 복사본»이니 손대지 않음. 등록 경로가 몇 개든 이 한 곳이 막는다. `pbRegisterFormMat` 에도 명시적 되살림.
- 공유문서 읽기 실패 → 이번 푸시에서 `fpm_shared` 는 'defer'(안 올림, 더티 유지). 통째 덮어쓰기 금지.
- 자동백업(abk·hbk) 스냅샷에 `priceBook`·`factories` 포함(이전엔 없어서 단가장은 백업으로 못 살렸다).
### 복구 도구 (데이터 관리 복구 창, 「단가장 품목만 복구」 파란 상자)
- ① `recoverPBFromItems()` — 아이템 부자재·원단이 기억하는 이름·규격·색·무게·온즈·오무데·단가로 없는 품목 재생성(라벨 제외) + 되살림.
- ② `recoverPBFromFile(input)` / `recoverPriceBookOnly(src,idx)` → `_pbUnionFrom(srcPB)`: 없는 것만 추가·단가 0이면 채움·빈 정보 채움·거래처 없으면 추가. **삭제 없음**.
### 아이템 카드
- 「단가장 등록」 버튼 삭제(부자재·원단·미등록 목록 전부) → 「단가장에 없음 · 열기」= `pbOpenBookFor` → `openBookWindow(scope,{sup,name})`.
- `openBookWindow` 가 `?bs=거래처&bq=품명#booktrim` 으로 열고, 새 창의 `renderBook` 이 `_pbApplyFocusFromUrl` 로 검색·거래처 필터·업체 펼침 후 그 줄 `.pbt-focus` 강조·스크롤. 「단가장에서 수정」 링크도 같은 경로.
### 부자재 단가장 표
- 단위 뒤에 **무게 g/㎡ · 온즈 · 오무데** 열(`pbSetMatField`, 다시 그리지 않아 커서 유지). `dl-pbomude`(우상·좌상+저장된 값). colspan 11→14. css `?v=20260907a`.
- 검증(실제 함수 10개, jsc): 되살림 전/후 병합·옛 복사본 제외·거래처 묘비·아이템 복구 2건·멱등·백업 합치기·단가 보호·새 창 포커스·칸 갱신.

## 2026-09-07b — 용량 진단의 「발주서 전체 합 100% 초과!」는 표시 오류
- 발주서는 08-18부터 8조각 문서라 전체 합을 문서당 1MB 한도와 비교하면 안 된다. 실제 한도 줄은 「8조각 중 최대」. 전체 합 줄은 참고용(막대·퍼센트 없음, `info:true`).

## 2026-09-07c — 아이템 카드 「단가장 등록」 복원 (+열기) · 「전부 단가장 등록」
- 요청: 단가장이 비어 다시 넣어야 하는데 카드에 이미 다 적혀 있으니 카드에서 등록하게 해달라. 07a 부터 되살림 표시가 붙어 안전.
- `pbRegisterFormMat` 부자재: `_TRIM_PB_LOCK_FIELDS` 값(규격·무게·온즈·오무데·색상·지퍼·단추…)과 유형(category)을 비어 있지 않은 것만 단가장에 올림.
- 미등록 목록 머리에 「전부 단가장 등록」(`pbRegisterAllUnreg`, 라벨 제외). 검증 4개(jsc).

## 2026-09-07d — 온즈가 자꾸 0.1 로 박히던 것
- 원인: 무게↔온즈 자동환산이 **키 하나 칠 때마다** 돌아서 «2»80 의 첫 키에서 온즈가 0.1 로 채워지고, 그 뒤엔 «빈칸이 아니라서» 안 고쳐졌다.
- 고침: `onWeightIn` 을 **칸을 떠날 때(change)** 만 돌리고, 자동으로 채운 값은 `data-auto=1` 로 표시해 다시 계산·비움. 직접 적은 값은 안 덮음.
- 정리: `_wgFixJunkOz` — 1 oz 미만이거나 무게와 50% 넘게 어긋난 온즈는 빈칸으로(아이템 원단·부자재는 부팅 정리, 단가장 품목은 saveData 훅). 정상값은 그대로.
- 검증 6개(jsc): 2→28→280 → 8.3 / 무게 지우면 자동 온즈 비움 / 직접 적은 값 보호 / 온즈→무게 281 / 쓰레기 정리 규칙.

## 2026-09-07f — 원단 「재단 방향」 (아이템 원단 줄 자료 → 작지 원·부자재 줄에 크게)
- 요청: 도식화에 재단 방향 칸 → 시안 ③(원단마다) 선택 + 「크게 잘 보여야」.
- 아이템 원단 카드 오무데 옆 「재단 방향」(`data-f=cutDir`, `dl-cutdir`: 양방향·일방향 ↓·일방향 ↑·결 맞춤·무늬 맞춤·스트라이프 맞춤·체크 맞춤, 직접 입력 가능). 단가장 잠금 아님(아이템 자료).
- 작지 `tpMaterialTableHTML` 원단 줄: 이름 줄 밑에 노란 띠 `.bom-cut`(「재단 **일방향 ↓**」 14px 굵게). 값 없으면 띠 없음(옛 작지 그대로). 검증: 실제 CSS 렌더 2띠·14px·배경.

## 2026-09-07g — 사이즈 없을 때 적은 치수(FREE)는 사이즈 추가 시 첫 사이즈로 자동 이관
- 신고: 사이즈 없이 치수를 적고 나서 사이즈 1·2를 추가하니 값이 사라진 듯 보임(실은 FREE 밑에 떠 있음, 되살리기 상자만 뜸).
- `tpSpecAutoAdoptFree(it,base)` — 치수 표를 그릴 때(`tpSpecSecHTML`) FREE 값이 있고 사이즈가 있으면 첫 사이즈로 옮기고 저장·토스트. 첫 사이즈에 이미 값이 있으면 안 덮음. FREE 아닌 옛 이름(XL 등)은 여전히 되살리기 상자.
- 수동 되살리기와 자동 이관은 `_tpSpecAdoptArr` 공용. 검증 5개(jsc).

## 2026-09-07h — 샘플·패턴 회차 치수 = 지시서 치수 공용 (측정부위 동일)
- 요청: 「지시서랑 샘플지시서 측정부위 동일하게」. 회차마다 spat.spec 을 따로 가져 부위 이름이 갈라졌음.
- 회차 패널이 `tpSpecSecHTML(it,'sew.spec',sk)` — 자료는 sew.spec 하나, 자수 빼기 등 시트 구분은 3번째 인자(sheet). 그레이딩은 원래부터 공용.
- `_tpSpecShareInit(sk)`: 지시서 치수가 비어 있고 회차에만 있으면 한 번 옮김(옛 회차 자료는 안 지움). 검증 4개(jsc).
- ⚠️ 회차별로 치수를 다르게 두는 기능은 없어짐(사용자 요청). 필요해지면 «회차 치수 따로» 토글로 되돌릴 것.

## 2026-09-07i — 「사라진 아이템만 되살리기」 (복구 창)
...
## 2026-09-08a — 로고·도식화가 «깨진 아이콘»으로 보일 때 진단·재시도
- 사진은 올리면 구글 저장소(firebasestorage) 주소로 바뀌고 다른 기기는 인터넷으로 받는다. 못 받으면 깨진 아이콘만 남아 이유를 몰랐다(신고 2026-09-08 아이패드).
- `_imgFailHandle`(document 'error' 캡처): 저장소 주소면 SDK `refFromURL().getDownloadURL()` 로 새 토큰 주소를 받아 1회 재시도 → 그래도 실패면 `img.img-fail`(빨간 점선)+토스트(묶어 1회 「사진 N장 못 불러옴 — 인터넷·로그인 확인」). `online` 이벤트에 재시도. dataURL·blob 은 손 안 댐.
- css `?v=20260908a`. 검증 5개(jsc).

## 2026-09-08c — 용량 진단이 인라인 사진을 포함해 「items 상자 100% 초과」로 겁주던 것
- 클라우드엔 `_stripDataUrlsDeep` 로 인라인 사진을 뺀 채 올라가는데 진단은 안 뺀 크기를 재서, 로그인 안 된 기기(사진 7장 2.2MB 인라인)에서 items_1 이 2023KB 초과로 표시됐다. 진단도 뺀 크기로. 인라인 사진 안내는 「사진 용량 정리」 상자가 맡는다.

## 2026-09-08d — 맥북 「로그인 칸도 로그인됨도 없음 · 동기화 멈춤 · 사진 못 올림 · 저장소 97%」 근본 원인
- 원인: `tryInitFirebase` 가 SDK(www.gstatic.com 스크립트 4개)를 **6초만 기다리고 영영 포기**했다. 와이파이가 느리거나 스크립트 하나가 잠깐 안 오면
  그 세션은 fbReady=false 로 살아서 ①연동 창에 로그인 칸·「로그인됨」 둘 다 안 보임 ②동기화 없음(아이템 안 옴·단가장 옛것) ③사진은 기기 안에만(인라인 7장 2.2MB) ④브라우저 저장소 97%.
  사용자는 "로그인은 돼 있는데 로그인하라 한다"로 봄 — 복구 창 문구도 그렇게 오해하게 써 있었다.
- 고침: 6초 뒤 **빨간 띠**(「구글 서버 연결 안 됨 — 계속 다시 시도 중」 + 지금 다시 시도/새로고침) 띄우고 2초마다 계속 기다림, 30초마다 스크립트 재삽입.
  일부만 온 SDK(auth/firestore 없음)는 초기화 전에 걸러 빠진 것만 재삽입·재시도. 이미 초기화된 앱은 `firebase.app()` 재사용. **진짜 연결된 뒤에만** 띠 내리고 「연결됐어요」.
  연동 창 not-ready 상자도 원인·해결을 적음. 검증 5개(jsc).
- ⚠️ 앞으로 «로그인하세요» 문구는 fbReady 가 true 일 때만. fbReady=false 는 «연결 안 됨»이다.

## 2026-09-08e — 저장공간 가득(맥북 97%)으로 편집이 조용히 사라지던 것 · 회차 치수 되살림 · 배색 칸 규칙
- 사고 경로: 인라인 사진 2.2MB → localStorage 가득 → `lsSet` 실패(메모리에만) → 작은 「저장 실패!」 글자와 토스트뿐 → 새로고침마다 편집(치수·사이즈) 소실.
- 고침: `fpfStorageBanner` 빨간 띠(「저장되지 않아요」+지금 비우기) · `_onSaveFailed` 가 **한 번 자동으로** `shrinkInlineImagesLocal()`(150KB↑ 인라인 사진 1200px·0.72 재압축) 후 재저장 · 성공하면 띠 내림.
  복구 창 노란 상자에 「사진 기기 안에서 줄이기」(클라우드 없이) 항상 표시, 연결 상태 문구도 fbReady 기준으로 정확히.
- 07h 보완: `_tpSpecShareInit` — 지시서·회차 둘 다 치수가 있으면 회차 값 중 지시서 «빈 칸»만 채우고 없는 부위는 추가(회차당 1회, `specMergedToSew`). 07h 가 회차 값을 숨겨 «사라진 것처럼» 보였던 것 되살림.
- `_tpHasSubMat`: 원단 2개여도 둘 다 «몸판»이면 배색 칸 없음(컬러별 원단). 용도 빈 원단 섞이면 예전대로 배색 표시. 검증 5개(jsc).

## 2026-09-08g — 라벨 카드에 「단가장 열기」
- 요청: 「케어라벨에서도 단가장으로 갈 수 있게」. 라벨 카드는 09-02 이후 「등록」·「✓단가장」을 안 보이지만, 단가장으로 «가는» 버튼은 필요.
- `_pbTrimBadge`: careLabel·mainLabel 이면 `pbOpenBookFor('trim',i)` 「단가장 열기」 버튼만 반환(등록 없음). 그 부자재처·라벨 이름(`_pbTrimName`)으로 단가장이 열리고 해당 줄이 강조됨. 검증 4개(jsc).

## 2026-09-08 사고 기록 — Storage 잠김(요금제)
- 로고·도식화가 전 기기에서 «깨진 아이콘»: 구글이 **Spark(무료) 요금제에서 기존 버킷 접근을 잠금**. 콘솔 Storage 화면에 「기존 버킷 액세스 복구하려면 요금제 업그레이드」만 표시(Rules 탭도 안 보임).
- Blaze 업그레이드 즉시 복구. 앱 코드·버킷 이름 문제 아님(storageBucket 변경 이력 없음). 사진 파일은 삭제되지 않았음.
- 같은 기간 맥북 증상(로그인 칸 없음·동기화 멈춤·저장 실패)은 별개 원인 = SDK 6초 포기(08d에서 고침) + localStorage 가득(사진 업로드가 막혀 인라인으로 쌓임). Storage 복구 후 「사진 클라우드로 정리」로 97%→54%.
- ⚠️ 교훈: 사용자 자료(사진·치수)를 «줄이거나 합치는» 자동 처리를 임의로 넣지 말 것. 08e 에서 시도했다가 즉시 되돌림(08f).

## 2026-09-08h — 「부자재 단가장에 하나라벨이 없어」
- 원인: 라벨 카드는 09-02부터 단가장에 품목을 넣지 않는다 → 하나라벨은 품목 0개 → `_pbSupKind`='none' → 부자재 단가장('trim'/'both'만 표시)에서 빠지고 원단 단가장 쪽에 있었다.
- 고침: 품목 0개(kind 'none')인 거래처는 **아이템 사용처**로 가른다(`_pbUsageKinds` 3초 캐시). 아이템 부자재·라벨만 쓰면 부자재 단가장에, 원단만 쓰면 원단 단가장에. `pb.kind` 를 직접 정해뒀으면 그게 우선, 품목이 생기면 예전 규칙.
- 검증 6개(jsc): 하나라벨 노출·원단 쪽 제외·원단 전용 유지·미사용 거래처 유지·kind 우선·품목 생긴 뒤.

## 2026-09-08i — 출고일 다 넣었는데 카드 위엔 「출고 미정」
- 원인 ①: 카드 헤더·정렬·디데이는 `it.sewingFcId` 의 일정만 봤는데, 아래 「출고」 칸은 `makerFc(it)`(=sewingFcId||promotionFcId)의 일정에 쓴다. 프로모션 아이템처럼 봉제공장 칸이 비면 서로 다른 곳을 보게 되어 «미정».
- 원인 ②: 헤더는 «예정일»만 봐서, 실제 출고 기록(shipments·shipDoneDate)만 있는 건도 미정으로 보였다.
- 고침: 행 구성에서 `sewingFcId = makerFc(it)`, `_eta2` 는 예정일 → 오더 출고일 → `fcLastShipDate`(회차·출고완료일) 순. 검증 5개(jsc).

## 2026-09-08j — 재단·출고 수량을 적었는데 카드엔 0
- 같은 뿌리(08i): `pdCutCSGrid`·`pdShipCSGrid`·행 집계가 `it.sewingFcId` 일정만 봐서, 프로모션 아이템(봉제칸 빔)이나 다른 공장 칸에 적은 수량이 0으로 보였다.
- 고침: `_pdSchList(it,o)` = 대표공장(makerFc) 먼저, 그다음 그 오더의 나머지 공장 일정. 결과가 나오는 첫 일정만 쓴다(중복 합산 방지). 아이템 필터(itemId)는 그대로.
- 검증 5개(jsc): 프로모션 출고 411·재단·다른 공장 폴백·다른 아이템 제외·중복 미합산.

## 2026-09-08k — 요척 히스토리가 사이즈별 요척을 무시하던 것
- 원인: 계획 요척을 `f.consumption`(대표값)만 썼다. 사이즈별 요척(`consumptionBySize`)을 쓰는 아이템은 계획이 실제와 어긋나 「-0.32」 같은 이상한 차이가 났다.
- 고침: `_yoPlanOf(fc,sizeQty)` — 그 발주에서 **재단한 사이즈 비율**로 가중평균(사이즈 정보 없으면 사이즈별 평균, 사이즈별이 없으면 대표값). 그룹 계획값도 장수 가중평균(`sumPlanW/sumCut`).
- 재단 기록도 `_pdSchList`(대표공장→다른 공장)로 찾는다 — 08i·08j 와 같은 뿌리.
- 검증 5개(jsc): 1.15 가중평균·실요척 0.88·장수·사이즈별 없을 때 1.2·다른 공장 폴백.

## 2026-09-08l — 원가계산서에 사이즈별 공임 반영 + 사이즈 기준 선택
- 신고: 사이즈별 공임(0=3,500 · 1=3,800)을 넣었는데 원가계산서에 「공임」 줄 자체가 없음. 원인: `pArr` 이 `it.laborCost`(기본값 0)만 보고 `if(!v)return null` 로 줄을 버렸다.
- 고침 ①: 공임 값은 `csLaborRate(it)` — 사이즈를 고르면 그 사이즈, 아니면 사이즈별 중 **최대**(원단 요척의 「최대 기준」과 같은 원칙). 라벨에 「· 최대 1 기준」/「· 0 사이즈」 표시.
- 고침 ②: 헤더 아래 **사이즈 기준 칩**(최대 / 각 사이즈). 고르면 `window._csSize` → 공임과 원단 요척(`effConsumption`)이 함께 그 사이즈로. 사이즈별 공임·요척이 있는 아이템에서만 보인다.
- ⚠️ 공임 단가는 `laborRate` 하나만 쓴다는 규칙 유지(결제·명세서와 안 갈라짐). 검산 14문제 통과. 검증 6개(jsc).

## 2026-09-08m — 요척 히스토리 사이즈별 상세 · 원가계산서 헤더에 사이즈 칸
- 지적: 「사이즈별 요척을 정확히 적어놨는데 왜 비율(가중평균)로 뭉개나」. 맞다 → 08k 의 뭉갠 계획 요척 표시를 버림.
- 사이즈별 요척이 있는 원단: 헤더에 **적어둔 값 그대로**(0 0.42 · 1 1.2), 아래에 「사이즈별 필요 야드」 표 = 사이즈 / 요척 / 재단장수 / 필요야드, 합계(계획 야드) · 실제 발주 야드 · 차이.
  단일 「추천 요척」은 내지 않는다(사이즈마다 요척이 달라 한 숫자로 못 씀). 사이즈별이 없는 원단은 예전대로 추천 요척.
- 원단은 통으로 발주하므로 실제 야드는 사이즈로 못 나눔 — 합계 비교임을 화면에 명시.
- 원가계산서 헤더: 품명 옆에 **사이즈 칸**(고른 사이즈 / 최대 기준). 사이즈별 공임·요척이 있는 아이템에서만 5칸.

## 2026-09-08n — 원가계산서 사이즈 기준이면 요척도 그 사이즈로
- `effConsumption` 은 08l 에서 이미 사이즈를 따르게 했지만, **요척 수정값(오버라이드)이 원단당 1개**라 한 번 적으면 모든 사이즈를 덮어써 사이즈를 바꿔도 요척이 그대로였다.
- `csFabKey(f)` = `f.id` + (사이즈 고르면 `@사이즈`). 사이즈마다 수정값을 따로 둔다. 「최대 기준」은 예전 키 그대로라 기존 값 보존. 검증 5개(jsc).

## 2026-09-08o — 아이템 수정에서 「저장」이 2~3번 눌러야 먹히던 것
- 원인: 부자재·원단 칸 일부가 `onchange="colTR();renderTrimRows()"` 로 **값 바뀌면 줄을 다시 그린다**. 입력칸에 커서를 둔 채 저장을 누르면
  mousedown → blur → onchange → 재렌더로 버튼 DOM이 사라져 **click 이 발생하지 않음**. 두 번째 클릭(포커스 없음)에서야 저장됐다.
- 고침: `saveItemFormSafe()` — 저장 버튼을 **pointerdown** 에서 처리. 먼저 `blur()` 로 대기 중 onchange(값 수집)를 끝내고 다음 틱에 `saveItemForm()`.
  pointerdown+click 중복은 0.8초 잠금으로 1회만. 아이템 폼의 두 저장 버튼 모두 적용. 검증 4개(jsc).
- ⚠️ 앞으로 폼 안에서 «값 바뀌면 재렌더»하는 칸 옆에 버튼을 둘 때는 click 대신 pointerdown 을 쓸 것.

## 2026-09-09a — 거래처 확인란: KG 원단도 「실제 야드」 입력
- 요청: 「오더 발주 거래처 확인란에 KG인 원단 야드도 적을 수 있게」.
- KG 원단은 돈이 kg×단가라 `actualOrderedQty` 가 **kg**. 야드는 `actualOrderedYards` 로 따로 저장(`setActualOrderedYards`).
- 입력칸: 협상 단가 표의 「실제 발주」 칸에서 kg 입력 아래에 **초록 y 칸**(KG 원단일 때만). 플레이스홀더 = 적은 kg × 야드/KG(없으면 계산 야드).
- `matActualYards(m)`: 적어둔 야드 → (kg 원단이면) 계산 야드 → (야드 원단이면) 실발주 → 계산 야드. **kg 값을 야드로 쓰지 않는다.**
- 쓰이는 곳: ①발주서 원단 줄의 야드 표시 ②요척 히스토리(실제요척 = 야드÷재단장수). **돈 계산은 불변**(kg×단가).
- 검증 6개(jsc) + 검산 14문제 87검사 통과.

## 2026-09-09b — 폰에서 「로그인 칸이 없다」 = 실은 회사 목록이 비었던 것
- 화면엔 「연동됨」·「클라우드 데이터 가져오기 완료」가 떠 있었으니 로그인은 정상. 회사 선택칸이 **빈 select** 라 작게 찌부러져 보였고, 그래서 아무 자료도 안 나왔다.
- 고침 ①: 클라우드 회사 목록이 비어 보이면 **2초 뒤 한 번 더 읽고**, 그때 있으면 그대로 이어간다(통신이 잠깐 흔들린 기기가 «새 사용자»로 오인돼 빈 화면 되던 것). 그래도 없을 때만 「첫 회사 등록」.
- 고침 ②: `renderCoSel` — 목록이 비면 「회사 없음 — 「새로고침」을 눌러주세요」 를 보여준다(빈 칸 금지).
- 검증 2개(jsc).

## 2026-09-09c — 「읽기 실패」를 「자료 없음」으로 착각하던 구조 (오늘 사고들의 공통 뿌리)
- 사실: `cloudGetDoc`·`cloudGet` 이 실패해도 `null` 을 돌려줬다. 부르는 쪽은 «클라우드에 없다»로 해석했다.
  → ①회사 목록 못 읽음 = 「새 사용자」 → 빈 화면 ②상자(items/tb/ref…) 하나 못 읽음 = 그 자료 없음 → 아이템·할일판이 사라진 것처럼 보임.
- 고침 ①: 두 함수 모두 **3회 재시도**, 최종 실패는 `_cloudReadFail[key]` 에 **기록**. 「문서 없음(exists=false)」과 구분.
- 고침 ②: 실패가 하나라도 있으면 «비어 있다»고 판단하지 않는다(회사 등록 모달 금지) + **푸시 차단**(`cloudSyncSave` 선두) — 반쪽 자료가 클라우드를 덮는 것 방지.
- 고침 ③: 주황 띠 「일부 자료를 못 받았어요 — 저장 잠시 멈춤 / 다시 받기」, `online` 이벤트에 자동 재시도.
- 고침 ④: 동기화 진단에 **사실**을 표시 — 「아이템 N · 오더 N · 거래처 N · 회사 N」, 「클라우드 읽기: 전부 받음 / N개 못 받음(문서명)+다시 받기」.
- 검증 5개(jsc): 정상·문서없음(실패 아님)·3회 재시도 후 기록·재성공 시 기록 삭제·회사목록 실패.

## 2026-09-09e — 발주서 상단 계산식에서 배색이 몸판 요척으로 나오던 것 (09-02 버그 재발)
- 09-02에 요약 줄(`fMap`)은 «이름+폭+용도»로 나눴는데, 펼쳤을 때 나오는 **계산식(`fFml`)은 이름만** 키였다.
  → 같은 원단을 몸판·배색으로 쓰면 한 덩어리가 되어 배색 줄에도 몸판 요척(1.2)이 표시됐다.
- 고침: `_fcG` 그룹 키를 `이름|용도` 로(용도는 `fc.part` 우선), `fFml` 키도 `이름|용도`, 줄에서 찾을 때 `이름|용도` → 옛 키 순으로 폴백.
  요척 폴백 계산도 그룹 대표 원단(`g.fc`) 대신 **그 자재의 원단(`fc`)** 사용.
- 검증 6개(jsc, 실제 코드 조각 실행): 용도별 2개 생성·몸판 1.2·배색 0.4·배색 40y·줄 매칭 2건.
- ⚠️ 같은 성격의 «이름만으로 묶는» 맵이 또 있으면 용도(part)를 반드시 키에 포함할 것.

## 2026-09-09f — 지운 오더가 되살아난 경로 2개 막음 · 생산수량 조정 «근거» 표시 · kg 실제야드 환산
- 신고: 「벌룬카고팬츠 어제 삭제했는데 폰·맥북 둘 다 살아남」. 코드로 확정한 되살림 경로:
  ① `_ordMissingCands`(복구 창 「사라진 오더 되살리기」)가 **삭제기록 있는 오더도 후보로** 보여줌 → 누르면 `_mt=now`+묘비 삭제 → 전 기기 부활. → 묘비 있는 오더 제외, 「내가 지운 오더 N건은 제외」 표시.
  ② `cloudSyncSave` 가 맨 위에서만 읽기 실패를 보고, **저장 직전 다시 읽는 4개 문서**(메인·발주조각·공유·보조상자)의 실패는 안 봄 → 반쪽 클라우드로 조각 덮어씀. → 읽기 직후 `cloudReadFailed()` 면 푸시 보류(주황 띠).
  확인된 오답: 실시간 오더 수신 후 로컬 저장(2345)은 `_tomb` 를 포함함(처음 의심했던 «묘비 디스크 유실»은 아님).
  ⚠️ 어느 경로가 실제로 발화했는지는 자료로만 확정 가능 — 「클라우드 실물 점검」에 «이 기기에만/클라우드에만 있는 오더·삭제기록 시각» 목록 추가.
- 신고: 「몸판 실발주 줄였는데 배색·시보리·부자재가 안 바뀜, 어떻게 돼서 바뀌는지까지 나와야」.
  `getEffectivePcsByColor(o,oi,trace)` 에 추적 배열(선택) — 판단하는 함수가 이유를 직접 남김(설명이 계산과 어긋날 수 없음).
  발주서 탭 자재표 상단 「조정 계산 — 아이템」: 실발주 ÷ 계획 = % → 컬러별 장수, 건너뛴 이유(계획 kg 0 · 부속 원단 · 컬러 불일치 · 차이 없음), 발송완료면 동결(6/26) 안내. 조정 0장이어도 실발주가 있으면 항상 표시.
- `matActualYards`: kg 원단이 실제 야드를 안 적었으면 «실발주 kg × (계획 y ÷ 계획 kg)» 로 환산(09-09a 는 계획 야드를 돌려줘 실발주가 발주서 합계에 안 보였음).
- 검증 9개(jsc, 실제 함수): 60→56장·추적 문장·계획 kg 0·부속 제외·trace 없이 동일·후보 제외·환산 3종. 가드 위치 정적 확인.

## 2026-09-09g — kg 원단 «실제 야드» 칸이 생산수량 조정에 연결 안 돼 있던 것 (09-09a 반쪽 연결)
- 사실: 발주서 본문 「※실제발주(원래 68y)」(16801)는 야드 칸을 보는데, 조정 계산 `getEffectivePcsByColor` 는 kg 칸(`actualOrderedQty`)만 봤다 → 야드를 64로 줄여도 배색·시보리·부자재 그대로.
- 고침: kg 원단은 야드 칸이 있으면 **야드 기준**(실야드 ÷ 계획야드 — 재단 장수는 야드가 정함), 없으면 kg 기준(예전 동작). `setActualOrderedYards` 가 `propagateEffectiveRatio` 호출. 「조정 계산」 블록 표시 조건에 야드 칸 포함. 돈(kg×단가)은 불변.
- 검증 6개(jsc, 실제 함수): 야드만·kg=계획+야드·kg만·둘 다 없음·저장 시 도미노 호출.
- ⚠️ 교훈: 실발주 입력 칸을 새로 만들면 **소비처 4곳**(조정 계산·발주서 본문·거래처 확인·요척 히스토리)에 모두 연결해야 한다. 한 곳이라도 빠지면 «표시는 바뀌는데 계산은 그대로»가 된다.
- 09-09h 보완: 발주서 탭 열 때 재계산 대상(`_ovItems`·`_ovIt`)도 야드 칸을 포함 — 이미 적어둔 야드가 열기만 해도 반영.

## 2026-09-09i — 웹 발주 카드 2단 (미리보기 → 사용자 선택: 세로로 둘 다 · 넓게 440)
- 요청: 「본문은 그대로 두고 내용선택·거래처확인을 오른쪽에 — 아래로 스크롤하는 피로감」.
...
## 2026-09-09l — 2단이 맥북에서 찌그러진 것: 창 폭이 아니라 «상세 패널 폭»으로
- 사고: 09-09i 의 `@media(min-width:1100px)` 는 창 기준. 맥북은 창은 넓지만 상세 패널(.sr)이 목록(360)·메뉴를 빼면 ~600px 라 440 오른쪽 열이 들어가며 왼쪽이 세로로 찌그러짐(스크린샷 확인). 카드엔 `.sc{max-width:540px}` 도 있음.
- 고침: JS `_po2ColMeasure()` 가 `#o-sr` 폭을 재서 트레이에 `.po2-2col` + `--po2-side` 를 붙임. 패널 ≥1000 → 440(사용자 선택) · 760~999 → 320 · <760 → 세로(예전). `resize` 마다 재적용. CSS 는 `.po2-2col` 로만 폭·grid·sticky 를 푼다. `?v=20260909b`.
- 검증: 측정 함수 3케이스(jsc) + 실제 CSS 하네스 렌더(1500/1150/900). 사용자 맥북(~640 패널)은 세로로 돌아가며, 창을 넓히거나 확대(zoom)를 줄이면 2단.

## 2026-09-09m — 요척 히스토리 묶음 2개·옛 사이즈 키(S·M)
- 「거즈 3중지」·「거즈 3중지 제작」 2묶음 = 같은 발주(8/13·예농·크림)에 **원단 자재 줄이 2개**(이름만 다름). 코드 경로: 발송으로 굳은 발주서(`refreshOrderSuppliers` 19177)는 아이템에서 원단 이름을 바꾸거나 지워도 옛 줄이 남는다. 히스토리 묶음 키가 `이름|용도|컬러`라 둘로 갈라짐.
  → 행에 오더명·거래처 표시, 같은 발주·용도·컬러 묶음이 2개면 주황 안내(「변경 사항」에 «필요 없음»으로 뜨는 줄이 그것). 자료는 안 건드림 — 정정은 2단계(정정 카드)에서.
- 옛 사이즈 키: 사이즈 S/M→0/1 교체 후 `consumptionBySize`·`laborBySize`에 S·M 값이 남아 머리글·「최대 기준」 계산에 섞였다. `_bySizeCurrent(map,it)` — 지금 사이즈 목록의 키만 표시·계산(요척 히스토리 머리글·원가계산서 effConsumption 최대·공임 최대·「최대 N 기준」 문구). 자료 삭제 없음.
- 검증 6개(jsc, 실제 함수).

## 2026-09-09n — 같은 원단을 두 용도로 쓸 때 요척 오염 · 발송 발주서의 «바뀌기 전 이름» 줄 삭제
...
## 2026-09-09p — 샘플 스케줄표 「텍스트 복사」 간단판
- 요청: 「간단하게 — 거래처별 아이템 (단계, 전달 9/9수 예정)만, 날짜 뒤 요일」. `ssBuildText` 를 머리 2줄(진행 현황·촬영 예정일) + 거래처(공장) 묶음별 목록으로 줄임. 촬영 불가/미정 섹션·소요 기간 안내·패턴/샘플 날짜 줄·「※ 촬영 불가」 삭제.
- `_ssMDW(iso)` = `_ssMD` + 요일(로컬 자정 기준 — UTC 밀림 방지). 검증 3개(jsc, 기대 문자열 완전 일치).
- 09-09q: 전체 훑기 — 원단 정의 찾기 8곳 중 7곳은 id 기준. 남은 1곳(`getEffectivePcsByColor` 컬러 역추적 `_fdef`)도 `_yoFabDef` 로 통일.

## 2026-09-10a — 샘플 스케줄표: 메인 발주로 넘어간 미발주 아이템이 사라지던 것
- `_ssRows` 미발주(i:) 줄 조건이 «phase sample && stage none» 이라 메인 발주가 생기면 빠졌다. 요청 「안 사라지게, 필요 없으면 내가 뺄 수 있게」.
- 고침: 스케줄표 기록(ssAct·ssShoot·ssDone·ssOff)이 있는 아이템은 메인/리오더로 넘어가도 줄 유지, `ph`=실제 단계라 「메인」 표시 + 「메인 N건 체크→빼기」가 그대로 동작. 기록 없는 메인 아이템은 안 올림(표 넘침 방지). 08-18 「자동 숨김 금지·체크로 빼기」 원칙 그대로. 검증 2개(jsc).
- 09-10b: 진행 현황 텍스트 줄 끝에 원단 발주 상태 `/원단발주완료 · /원단 발주예정 · /수량 미정` (`_ssFabPo`: 메인 오더의 원단 거래처 발주서 발송·출고·입고 여부 / 오더 유무). 검증 4개.
- 09-10c: 스케줄표 단계 라벨 「샘플 완료」→「투입완료」(samOut 기록 시). `_ssRemain` 분기에 두 문자열 모두 인식. 검증 3개.
- 09-10d: 스케줄표에 「원단 발주」 칸(지금 단계 옆) — 완료(초록)/예정(노랑)/미정(회색), `_ssFabPo` 재사용. 묶음 헤더 colspan 12. `?v=20260910a`.
- 09-10e: 스케줄표 머리글 sticky(≥900px에서 `.ss-tw` overflow 풀고 `thead th{position:sticky;top:0}`). 좁은 화면은 가로 스크롤 유지. `?v=20260910b`.

## 2026-09-10h — 부자재 단가장에 「아루드」(브랜드)가 생긴 것
- 경로: 메인라벨은 브랜드가 주므로 부자재처=브랜드명(16115) → 그 발주 카드에서 옵션·협상가 저장 시 `S.priceBook['아루드']` 자동 생성(16424·20275, 품목 0) → 09-09h 「품목 0 거래처는 아이템 사용처로」 규칙이 부자재처로 올림.
- 고침: `_pbUsageKinds` 에서 메인라벨 줄은 제외 · `_pbIsBrandName` — 브랜드 이름과 같은 품목 0 항목은 단가장 양쪽 다 숨김(품목이 생기면 보임). 자료 삭제 없음. 검증 4개.

## 2026-09-10i — 부자재 단가장 유형 바로가기
- 업체 띠 아래(고정 머리 thead)에 유형 칩 줄(`tr.pbt-cats`, 유형이 2개 이상일 때만). 각 유형 띠(`tr.pbt-cat`)에 `id="pbtcat-<업체>-<n>"`, `pbtJumpCat` 이 가운데로 스크롤 + 1.6초 노란 강조. `?v=20260910e`. 검증 3개.

## 2026-09-10j — 단가장 이름 변경이 아이템·발주서를 다 따라가게 (적용컬러 소실 사고)
- 사실: `renamePBMaterial` 의 아이템 갱신이 «색상까지 같을 때만»이라 단가장 색상 빈 항목 ↔ 아이템 색상 있는 줄이 안 맞아 아이템이 옛 이름으로 남음 → 카드 미등록 → 다시 고르면 새 줄 → colorLinks(발주용 컬러명) 소실. 발주서는 갱신 대상 아님.
- 고침: 단가장 색상이 비면 색상 무관 매칭(줄은 그대로, 이름만) · 발주서 자재도 같이(발송·출고·입고분은 이름·표시명만, 수량·실발주·`skipBilling` 키 이관) · 토스트에 「아이템 N개 · 발주서 M장 같이 바뀜」. 검증 8개.

## 2026-09-10k·l — 「창을 키워도 2단이 안 됨」 진짜 원인: 분할 요소가 창 폭을 안 채움
- `#pane-orders` 는 `class="split"` 뿐이라 가로 플렉스 부모(.content) 안에서 내용물 크기만큼(카드 560+여백 ≈ 600)만 차지 → 창이 넓어도 상세 패널 고정, 오른쪽 빈 배경. 어제 검증 페이지는 실제에 없는 `.panel` 래퍼를 넣어 못 잡았음(교훈: 검증 페이지는 실제 DOM 구조를 그대로).
- 고침: `.content>.split{flex:1 1 auto;min-width:0}`(10k). 2단 기준 완화 700→320 / 900→440(10l), JS 폴백도 동일. `?v=20260910g`.
...
## 2026-09-11e — 원단처 바꿔도 동·층·호수가 안 바뀌고, 옛 주소가 새 거래처에 덮이던 것 (치우→유니콘)
- 원인 ①: `autofillFab` 이 «옛 원단처»를 `formFabs`(메모리)에서 읽는데, 임시저장 타이머가 `colFR()` 로 메모리를 먼저 새 값으로 바꿔 «안 바뀜»으로 오인 → 빈 칸만 채워 옛 주소 잔존. → 옛 원단처를 **줄 자체(`data-af-sup`)** 에 기억·비교.
- 원인 ②: `saveToPB` 「단가장 주소가 비면 카드 주소로 채움」이 잔존한 옛 주소를 **새 거래처(유니콘)에 써넣음** → 이후 유니콘을 고르면 치우 주소. → `_pbAddrOwner`: 카드 주소가 다른 거래처 것이면 복사 안 하고 알림(사용자 규칙 「동층호수는 중복될 수 없음」).
- 표시: 단가장 업체 띠·원단 카드 연락줄에 「주소 중복: ○○와 같음」 경고 → 지금 잘못 들어간 유니콘 주소를 정보에서 고치면 사라짐. 검증 6개(jsc, 실제 함수).

## 2026-09-11g — 오더 상세 폭: 윗부분 540 복귀, 발주 카드 묶음만 넓게
- 11a 에서 `.od-body` 를 1160 으로 풀었더니 탭·머리·수량표·로스·변경 사항이 제각각 넓어짐(사용자: 「규격 맞추고, 윗부분은 원래대로 좁게」).
- `css/main.css`: `#o-sr .dtabs`·`.od-body>*`·`#od-tab>*` = 540, **`#od-tab>.po2-cq` 만 max-width 없음**(2단 1120 까지). `?v=20260911b`.

## 2026-09-11h — 거래처 확인 깨짐(담당자·협상단가 겹침) 수정
- 원인: 2단 그리드 선택자 `.po2 .sc-b`(자손) 가 **거래처 확인 안쪽 작은 카드의 `.sc-b`** 에도 걸려 그 안이 `1fr 440px` 2열로 쪼개짐 → 첫 열 폭 0 근처(담당자 라벨이 옆 칸 위로 넘침, 「협상 단가」 제목이 세로로 한 글자씩).
- 수정: `.po2>.sc-b`(자식만) — container query 블록·`@supports not` 대체 블록 둘 다. `?v=20260911c`. 하네스 `po2col-real.html` 에 안쪽 카드(`#innerck`) 추가해 display:block 확인.
- 규칙: **발주 카드 안에 또 `.sc` 카드가 들어가므로 `.po2` 아래 구조 선택자는 반드시 자식(`>`) 로.**
- 정돈안 미리보기(승인 대기): 왼쪽=본문만 / 오른쪽=보내기 설정·자재·내용 선택·거래처 확인 같은 머리띠 4섹션. https://claude.ai/code/artifact/48207704-d107-4ef7-b654-ccb3178612c5

## 2026-09-11i — 발주 카드 정돈안 적용 (사용자 승인 「이대로 진행해줘」)
- **왼쪽 `.po2-main` = 발주서 본문만** / **오른쪽 `.po2-side` = 거래처 확인 → 자재 N건 → 발주 내용 선택(맨 위 그룹에 부자재 수량 단위)**. 2단(컨테이너 ≥700)은 **50:50**, 카드 `.sc-b` padding 0·gap 0, 오른쪽 왼선 2px.
- 카드 머리의 **출고처 select 삭제**(거래처 확인의 출고처와 같은 값이라 중복). 대신 `renderCheckTab` 출고처 줄을 **모든 거래처**에(전엔 원단처만).
...
- 2026-09-11k: 발주 내용 선택의 칩·부자재 단위·안내 상자 테두리 1px(그림자 없음), 켜짐=민트. css `?v=20260911f`.

## 2026-09-11l — 로스/여분을 거래처 카드 안 「발주 내용 선택」에 (사용자: 맨 위에만 있으니 쓰기 불편)
- `orderLossCardGroupHTML(o,sn)`: 그 거래처(`(추가발주)` 접미사 떼고 비교) 자재만 줄로. 저장 자리는 기존과 같은 `o.loss[itemId][matId]` → 위 패널과 같은 값. 추가발주 orderLike(`S.orders` 에 없음)는 그룹 없음.
- 카드 안 프리셋 칩 = `applyLossPreset(oid,idx,sn)` 세 번째 인자로 **그 거래처 자재에만** 적용. 프리셋 수정·전체 적용은 맨 위 패널 그대로(기본 접힘).
- 검증: JSC 하네스(진짜 함수 추출) — 거래처별 행 필터·추가발주 키·없는 거래처 빈값·프리셋 거래처 한정/전체 6항목 통과. css `?v=20260911g`.

## 2026-09-11m — 치수 시트 겹친 부위 정리 (09-08e 회차 합치기 오류 뒷정리)
- 증상: 샘플 코멘트 사이즈표에 같은 부위가 3번. 원인: 코멘트 표는 작지 `sew.spec` 을 그대로 비추는데, 09-08e(되돌린 코드)가 회차 치수를 sew.spec 에 덧붙여 놓은 자료가 남아 있음.
- `tpSpecDedupeRows(arr)`: 부위 이름 기준 첫 줄만, 첫 줄 빈 칸은 뒷줄 값으로. `tpSpecDupCount` 가 0 이 아니면 작지 치수 탭 머리에 **「겹침 정리 N」** 버튼(주황) — 확인창 뒤 `tpSpecDedupe(base)` 로 정리(자료 삭제는 확인 필수).
- 코멘트 쪽: `_scSizeAuto` 도 부위당 한 줄(첫 줄 우선). 이미 저장된 `rr.size` 는 **실측이 빈 겹침만** 화면에서 한 번 — 자료는 안 지움.
- 검증: JSC 하네스 5항목(겹침 수·정리 결과·첫 줄 값 유지·자동 줄·저장 줄 표시) 통과.
- 2026-09-11n: 「겹침 정리」 버튼 삭제(사용자: 「애초에 겹치지 않게」). `tpSpecAutoDedupe(it)` 를 **로드 때 전 아이템(migrateData) + 작지 열 때(tpOpen, 바뀌면 저장)** 자동 실행. 시트마다(sew + 회차) 첫 줄만 남기고 빈 칸은 뒷줄 값. 하네스 4항목 통과(5줄 정리·값 유지·회차 시트·두 번째 실행 0).

## 2026-09-11o — 샘플 코멘트 사이즈표: 컬러별 실측·차이 칸(A안) + 줄 이동
- 컬러 2개 이상이면 `_scSizeTableHTML` 이 머리 2줄(측정부위·패턴·지시 | 컬러마다 실측·차이)로. 컬러 1개면 옛 표 그대로.
- 저장: 첫 컬러 실측 = `r.meas`(옛 자료 호환), 나머지 = `r.mc[컬러명]`. `scSetSizeC(id,ri,i,ci,col,v)`. 행 복사는 `_scRowCopy`(mc 보존), 불러오기(`scSizeTake`)는 `_scRowHasMeas` 로 컬러별 실측도 지킴. 화면 겹침 숨김도 `_scRowHasMeas` 기준.
- 줄 이동 `scSizeMove(id,ri,i,dir)` ▲▼ (× 옆). 작지 코멘트 창(`#tp-cmt-body`)도 같은 함수라 동일. css `?v=20260911h`(컬러 칸 색 scc0~3).
- 검증: JSC 하네스 5항목(머리·입력칸 수·컬러별 저장·불러오기 유지·이동·컬러 1개 무변화).
- 2026-09-11p: 코멘트 사이즈표 줄 순서 = **손잡이(⠿) 끌기**(`scGripStart/Move/End`, 작지 `tpGrip*` 과 같은 포인터 방식 → 아이패드 손가락 가능) + × 삭제. ▲▼ 는 칸이 좁아 ×가 밀려 안 보여서 제거. 마지막 칸 58px. css `?v=20260911i`.
- 2026-09-11q: 컬러별 사이즈표에서 지시·패턴을 고치면 컬러별 차이 칸(`sc-diff-ri-i-ci`)도 즉시 재계산(`scSetSize`). 실측 먼저 적고 지시를 나중에 적으면 차이가 빈 채로 남던 버그.

## 2026-09-11r — 작지 치수 «측정부위» 시트끼리 통일 (값은 시트별 그대로)
- 사용자: 「치수를 샘플에서 변경하면 측정부위는 통일되어야 해, 안 내용은 아니더라도」. 09-08f 「회차는 자기 치수」 결정은 유지(값 별도)하되 **부위 목록·순서만** 맞춘다.
- `tpSet` 훅: 경로가 `<시트>.spec` 또는 `<시트>.spec.N.part` 면 `tpSpecSyncParts(시트, {from,to})` — 다른 시트(sew·grd·spat 회차)를 원본 부위 순서로 재배열, 같은 이름 행의 값 보존, 이름 바꿈은 다른 시트도 같이 바꿈. 값만 바뀌는 경로(`.pat.`/`.v.`)는 동기화 없음.
- 자료 보호: 원본에 없는 부위라도 **값이 있으면** 뒤에 남김. 확인창을 거친 줄 삭제(`tpDelRow`)·프리셋(`tpSpecPreset`)만 `_tpSpecForceDrop` 로 진짜 삭제. 확인창은 다른 시트 값 칸 수를 보여줌.
- 지금 어긋나 있는 아이템은 다음에 어느 시트든 부위를 건드리는 순간 그 시트 기준으로 맞춰짐(자동 일괄 변경은 안 함 — 어느 쪽이 기준인지 사용자가 정하게).
- 검증: JSC 하네스 2벌(추가·이름·순서·삭제·값만 변경 / 값 보호·확인 후 삭제·프리셋) 통과.
- 2026-09-11s: 카드 안 로스/여분 프리셋 칩 = `.po2-chip`(다른 칩과 같은 글꼴·색·크기). 이 거래처의 해당 자재가 전부 그 프리셋 값이면 `on`(민트). 옛 `.lp-chip` 안 씀.
- 2026-09-11t: 발주 내용 선택의 2.5px 테두리+그림자 = 1379줄 `#o-sr .po-details … !important` 규칙. `#o-sr .po2-side .po-details` 로 더 센 규칙 추가해 제거, 칩·단위·로스 입력칸 1px. css `?v=20260911j`.
...
## 2026-09-14e — 메인라벨 발주처 = 브랜드 (옛 부자재처 「메이드」 남는 문제)
- 신고: 메인라벨 카드 머리에 「메이드」. 원인(코드): ① `onTrimType` 로 다른 부자재를 메인라벨로 바꾸면 `supplier:t.supplier` 를 그대로 가져감 ② 메인라벨 카드는 부자재처 칸이 숨은 칸이라 안 보이고 못 고침 ③ `calcSups` 가 부자재처가 **비었을 때만** 브랜드로 채워 → 그 메인라벨이 메이드 발주서로 들어감.
- 수정: 브랜드가 있으면 발주처 항상 브랜드(calcSups) · 타입 바꿀 때 부자재처 비움 · 브랜드 고르면 부자재처=브랜드명 · 카드 머리 = 브랜드명(없으면 「브랜드 미정」) · 불러올 때(migrateData) 브랜드 있는 메인라벨의 옛 부자재처 정리. 보낸 발주서는 불변.
- 검산 문제 17(3검사).
- 2026-09-14f: 메인라벨 발주처 = 카드 브랜드, **없으면 아이템 브랜드**(calcSups·migrateData·카드 머리). 신고 「메인라벨이 메이드에 가 있어」 — 14e 는 카드에서 브랜드를 고른 줄만 고쳤음. 또 거래처 이름 바꾸기/합치기(7072)·단가장 품목 합치기(9378)가 메인라벨의 부자재처(브랜드명)까지 바꾸던 경로를 막음. 검산 17 에 2검사 추가(115).

## 2026-09-14g — 아루드 케어라벨 제조연월 기본 「표시 안 함」 · 치수 가져오기 창 레트로 · 「연동됨」 버튼 모양
...
## 2026-09-15a — 협상가: 단가장에 따로 기억 · 새 오더 자동 · 결제만 협상가 · 원가는 매장가
- 사용자 결정: 「새 오더에는 네고가 자동, 발주서 @15,000>14,000, 결제 14,000, 원가 15,000 기존 매장가 그대로」.
- 예전(코드 확인): `setNegotiatedPrice` 가 단가장 매장가(unitPrice/pricePerKg)를 협상가로 **덮어씀** · 아이템 단가 불변 · 리오더 협상가 칸 빈칸.
- 지금: 단가장 항목에 `negotiatedPrice`·`negoUnit`('kg'|'unit')·`negoAt`(정한 날) 따로 저장, 매장가 안 건드림(0 이면 지움). 원단은 `_fabE`, 부자재·라벨은 `_pbFindTrimMat` 로 진짜 항목을 찾음(예전엔 이름 키로 가짜 항목 생성). `(추가발주)` 접미사 떼고 찾음.
- `_pbNegoFor(o,sn,m)`: 오더 createdAt ≥ negoAt · 단위 일치 · 매장가와 다를 때만 → `_refreshOrderSuppliersRaw` 복원 단계에서 사용자 입력 협상가가 없으면 채움(m.negoFromPB). 보낸 발주서·옛 오더 불변.
- 결제 `getMatActualCost`=협상가 · 발주서 `_poAt`=@매장가>협상가 · 원가(원가계산서·calcFabYards)=아이템 매장가 → 기존 그대로.
- 주의: 이번 전에 적은 협상가는 이미 단가장 매장가를 덮어쓴 상태(원래 매장가는 모름) — 사용자에게 알림.
- 검산 문제 19(7검사, 132).

## 2026-09-15b — 원단 카드 새 배치 (사용자 승인 「원단 카드 진행해」)
- `.fab-split` = `minmax(0,1fr) 270px`. 왼쪽 칸 안에 비고 아래로 **적용 컬러(colorSec)·컬러 이름 가산(fabKwSecHTML)** 이동. 오른쪽 = 혼용률(제목 옆 합계 #comp-sum-i, `.fab-comp-scroll` max-height 158px 안에서 스크롤) + ＋ 혼용성분 추가 + **큰 스와치 `_fabSwBig`**(240px 정사각형·썸네일 4장·+N·＋, 단가장 미등록/사진 없음 안내). 원단명 옆 작은 스와치 칸(_fabSwThumb) 삭제. 가공비·가산 항목은 카드 아래 전체 폭. 760px 이하 위아래로 쌓임.
- 검증: 실제 fabRowHTML 출력 + 실제 CSS 렌더 측정(1200px: 왼 811/오 270 · 컬러·가산 왼쪽 안 · 혼용률 6줄 스크롤 · 스와치 240×240). css `?v=20260915a`.

## 2026-09-15c — 생산 대시보드 오더›재단›출고 표에 비품 출고
- 사용자: 「비품 출고도 위 표에 — 재단수량이랑 다 만든 게 맞는지 확인용」.
- 출고 기록 중 `bg`(비품 회차)를 `bgTotal/bgColors/bgSizeGrid` 로 따로 집계(출고 합계 shipTotal·진행률은 그대로). 칸 = 오더 › 재단 › **출고(정상=전체−비품, 파랑)** · **비N(빨강)**. 위 요약 「출고 650 · 비품 33」, 범례에 「출고(정상)+비품이 재단과 같으면 다 만든 것」. css `?v=20260915b` `.bgq/.bgs`.

## 2026-09-15d — 출고 회차 = 날짜 + 구분(정상·비품·잔량·수선)
- 신고: 같은 날짜에 정상·비품이 나가면 비품 체크 시 전부 비품이 됨. 원인(코드): `pdOpenQtyModal` 이 지난 회차를 **날짜만**으로 묶음(byDate) → 같은 날 정상+비품이 한 회차, 수정하면 비품 토글이 켜진 채 합친 수량이 채워지고 `pdGridSave` 가 그 날짜 기록을 통째로 교체(삭제도 날짜 전체).
- 수정: `_pdRecKind`(bg/lv/rp/n)·`_pdRoundKey`(날짜|구분)·`_pdKeyMatch` — 회차 목록·수정 채움·저장 교체·삭제가 모두 날짜+구분 기준. 날짜만 담긴 옛 키도 동작.
- 2026-09-15e: 날짜로 묶기 자체 제거(사용자 「날짜별로 묶어버리는 것 좀 없애줘」). `pdGridSave` 가 저장마다 회차 번호 `bid` 를 붙이고(재단 replace 제외), `_pdRoundKey` = bid 우선·없으면 옛 날짜|구분. 목록은 기록 날짜순·같은 날은 입력순, 라벨은 회차의 date. 삭제·수정도 bid 기준. 옛 키로는 bid 회차를 안 건드림.

## 2026-09-15f — 부자재 가공비: 담당 집 발주서 · 금액 표시 (사용자 결정 D)
- 부자재 가공은 보통 부자재 산 집이 함 → 담당(tc.factory) 비면 부자재 산 집. `_poTrimProcNotes(g,sup)`: 담당이 이 거래처가 아니면 뺌 · 컬러당/한번에 금액 「(2,300원 · 컬러당)」. 개당·조건부 금액은 `_poProcBlocks`(부자재는 담당 비면 부자재 산 집으로 인정).
- 담당이 다른 집이면 `calcSups` 가 그 집에 `type:'proc'` 자재(금액 0 — 가공비는 원가 trimCostPerPcs·가공비 결제 줄에서 집계, 이중 계산 금지)를 넣어 발주 카드가 생김. `genPoText` 가 「가공: 이름 (금액)」 · (자재 — 산 집에서 받음) · #컬러(개당이면 개수). 거래처 확인 협상단가 표에선 proc 줄 숨김.
- 검산 문제 20(5검사).

## 2026-09-15g — 옛 출고 기록도 저장 단위 회차로 풀기 (사용자 「다 풀어줘」)
- 15e 전 기록은 bid 가 없어 계속 날짜로 묶였음. `_pdSplitLegacyRounds(arr)`: 배열 순서대로 같은 칸 반복·칸 순서 역행·날짜/구분/메모 변화 시 새 회차 → `bid='L'+첫 기록 id`. 출고 입력 열 때(바뀌면 저장)와 migrateData 에서. 멱등. 안내문 「저장하면 새 회차로 따로 추가돼요」.

## 2026-09-15h — 정정 카드 2단계 (만들기 · 본문 차이만/전체 다시 · 발송완료 시 실제 발주 자동 채움)
- 저장 `o.poCorr[거래처]=[{id,n,createdAt,status draft|sent,sentAt,mode delta|full,lines}]` — o.suppliers 불변(결제·재계산 경로 안 건드림).
- 변경 사항 패널: 비교 기준 = 보낸 발주서 + 보낸 정정(`_poCorrBase`/`_poCorrApplyTo`), 거래처 줄에 「정정 카드 만들기 / 다시 계산」(`poCorrCreate` — 미발송 초안이 있으면 줄만 다시 계산).
- 카드(`poCorrCardsHTML`, 원래 카드 바로 아래, 주황 왼줄): 머리 복사·카톡·발송완료·지우기 / 왼쪽 본문(`poCorrText` = 정정 반영 자재로 genPoText → `_poCorrCompose`) / 오른쪽 바뀐 자재 + 정정 본문 칩(차이만·전체 다시, 거래처 poOpts.corrMode 로 기억).
- 차이만 본문: 상호·품명(+「정정 (M/D)」) · ※ 안내 · (용도) 이름 · #색 - 15y → 14y (-1y) / (새로 추가) / → 취소 · [출고처]. 전체 다시: ※ 안내 + 전체 발주서.
- 발송완료(`poCorrSend` → `_poCorrFill`): 실제 발주 칸이 비었거나 직전 수량 그대로면 정정 수량(KG 원단은 kg + 실제 야드), 취소 줄은 0, 새로 추가 줄은 자재 추가. 직접 적은 숫자는 그대로. 원단이면 propagateEffectiveRatio.
- 검산 문제 21(7검사, 144). css `?v=20260915c`.
- 2026-09-15i: 출고 입력창 지난 회차 줄을 누르면(▸/▾) 그 회차에 적은 컬러×사이즈 수량 표(`pdToggleRoundDet`, 줄 바로 아래 `.pdr-det`). 수정·삭제 버튼은 그대로.

### 15j. 원단 카드 스와치 끌어다 놓기 (2026-09-15)
- 카드 오른쪽 스와치 칸(`.fabsw-wrap`)과 스와치 창(`.fabsw-dlg`)에 사진 파일을 끌어다 놓으면 올라감. 올리는 길은 「＋ 사진 올리기」와 같은 `mtAddPhotos`(단가장 원단 항목 swatches).
- 끌고 있는 동안 칸 강조(`.fabsw-drag`: 하늘색 #B0DBF0 + 검정 테두리). 글자 끌기는 무시(`dataTransfer.types`에 Files 있을 때만).
- 스와치 큰 사진 비율 3:4(세로로 길게), 사진 추가 ＋ 버튼은 높이 20px 얇게.

### 15k. 샘플 확인서 사이즈표 — 같은 차수 작지에서 가져오기 · 손잡이/× 겹침 (2026-09-15)
- 확인서 N차의 패턴·지시는 작지 「샘플·패턴 N차」 치수(패턴=pat, 지시=완성 v)를 기본으로 쓴다(`_scPairKey`). 짝 차수가 없으면 옛 방식(값이 가장 많이 적힌 시트).
- 출처 없이 굳은 옛 표라도 실측이 한 칸도 없으면 같은 차수 작지 치수로 보여준다. 실측이 있으면 그대로(「불러오기」로 바꿈).
- 표를 처음 고칠 때 출처(sizeSrc·sizeSz)도 같이 저장 → 제목 「샘플·패턴 N차 · 사이즈」가 실제 값과 일치.
- 사이즈표 오른쪽 칸(`td.sc-ctl`)은 툴바 `.sc-ctl`(flex)과 이름이 같아 ×가 줄 경계로 밀렸음 → 표 칸(table-cell)으로 되돌리고 × 제 폭. 인쇄·이미지 저장 숨김은 그대로.

### 15l. 원단 가공비 = 원단 요척 · 줄 세 덩어리 (2026-09-15)
- 신고 「야드당 2,000원인데 원가 +2,000원/장이면 안 되지 — 요척에 맞게」. 원단 가공비(tc.fabId)의 「y/장」 칸이 비면 1로 곱하던 걸 **원단 요척**으로(`_tcQtyPerPiece(tc,trims,fabs)` → `_tcFabYo`). 요척 = consumption(사이즈별이면 가장 큰 값) — 원가계산서 원단비와 같은 기준.
- 원가 칩·원가계산서(표·내보내기)·장당 원가·결제 명세(단위 y)·거래처 원장(단위 y)·결제 화면·발주서 가공 블록이 모두 같은 함수. 부자재 가공비는 그대로.
- 옛 자료: 원단 가공비(개당·조건부) 칸에 1 이 적힌 줄은 비움(=요척)으로 한 번 바꾸고 옛 값은 `tc._qppOld`에 남김(`_migrateFabProcYo`, 줄마다 `_yoMig`). 첫 실행 때 안내 토스트 1회.
- 폼: 원단 가공비 줄만 세 덩어리 — 누가·무슨 가공(이름·종류·가공하는 집) / 가공비(금액·야드당·컬러별·한번에·조건부 버튼·요척·가공집 최소·계산 한 줄) / 원단 더 시키기(옷 1장마다 y·합쳐서·「100장이면 +Ny」). 요척 칸은 비우면 회색(원단 요척), 직접 적으면 노랑.
- 검산 문제 22.

### 15m. 원단 카드 「컬러별 단가」 상자 — 야드단가 밑 (2026-09-15)
- 사용자 「컬러 가격은 후가공이 아니니까 위 원단 쪽에」. 원단명·폭·**기본 야드단가** 줄 바로 밑 파란 상자(`fabPriceBoxHTML`): 한 줄 = 「컬러별 단가 | 이름 가산 [멜란지|+500|×] … [글자][+금액][추가]」, 아래 = 체크한 컬러마다 [이름 | 직접 단가 칸 | 최종 원단 단가] 격자(주황 = 이름 가산 붙음, 노란 칸 = 직접 적음).
- 「적용 컬러」 상자는 체크 + 원단처 색상명만. 컬러 이름 가산 노란 상자(옛 `fabKwSecHTML`)는 삭제.
- 저장 자리·계산 불변: 직접 단가 = colorLinks[].price(`data-fab-col-price`, colFR), 이름 가산 = 단가장 원단 항목 colorKwAdds. 글자·금액 칸이 따로라 「멜란지 -500」 파싱 오류 없음(금액 0 이하면 안내만).
- 기본 단가·색상명·직접 단가를 칠 때 최종 숫자만 다시 계산(`fabCpRecalc`, 포커스 유지). 레트로 테마의 원단 카드 입력 흰 배경(!important)은 이 칸들만 되돌림.

### 15n. 가산 항목 = 후가공 · 금액 기준 · 더 시킬 야드 / 작지 코멘트 칸 2배 (2026-09-15)
- 가산 항목 줄 칸: 이름 · 더할 금액 · 축률 % · **더 시킬 야드** / 버튼: **금액 기준(야드당·컬러별·한번에)** · 적용(전체 컬러·선택 컬러만). `basis`('yard' 기본)·`extraYd` 저장.
- 야드당 = 원단 단가에 더함(`calcFabAdditionForColor`는 야드당만). 컬러별·한번에 = 원단처 정액 → `_fabAddTcs(it)`가 가공비 줄(perLot·once, 담당=원단처)로 만들어 `_tcAll(it)`로 원가계산서·장당 원가·결제 명세·결제 화면·거래처 원장·원가 내보내기·발주서 「가산: 코팅 (20000원 · 컬러당)」 줄이 같이 탄다.
- 더 시킬 야드 = 로스·축률 뒤 컬러마다 그대로 더함(`calcFabExtraYdForColor`, 컬러를 줄 때만 — 원가용 계산엔 안 더함). 자재 `extraYards` 기록. 원단 부족 역산(`getEffectivePcsByColor`)은 이 야드를 빼고 비율 계산. 발주서 = 합친 야드 + 「※ 컬러마다 앞뒤 여유 2y 포함」(사용자 「가」). 품명 줄 옆 이름은 야드당 금액 있는 줄만.
- 작지 「코멘트 열기」 오른쪽 칸 폭 340px → min(680px, 50vw)(좁은 화면 min(680px, 94vw)).
- 검산 문제 23.

### 17a·17b. 작지 — 「메인 지시서」 제목 · 개선사항 시트 (2026-09-17)
- 지시서(sew) 시트 제목: 작업지시서 → **메인 지시서** (`titlePfx` + 탭 전환 시 `tp-pfx` 갱신 두 곳 다).
- 새 시트 **개선사항**(key `qc`, 탭 위치 = 안사양 뒤): 제목 「메인작업지시서(개선사항)」, 내용 = 퀄리티 확인 사진 4칸(`qcFront`·`qcBack`·`qcLining`·`qcDetail`, 샘플 사진 시트와 같은 `_smpSlot` 재사용 · 클릭 업로드 · 끌어다 놓기 · ×삭제) + 개선사항 메모(`qc.note`).
- 등록 지점 4곳: `_validTabs`, `PROC`, 탭 문자열(T('qc','개선사항')), `doc.innerHTML` 패널 연결. 인쇄 때 사진 7.8cm(가로 5.2cm)로 줄여 메모까지 A4 한 장.

### 17c. 작지 라벨 사진 밑 「라벨 위치」 한 줄 (2026-09-17)
- 라벨 실물 사진 표 바로 밑에 얇은 한 줄 추가: `[라벨 위치] [입력칸]` (`.tp-lblpos`, 저장 자리 `sew.labelPos`).
- `sewLabelTbl` 에 붙여서 세로·가로 배치와 안사양 시트(같은 sew.* 자료를 비춤)에도 같이 나온다. 라벨 사진이 없으면 표와 함께 이 줄도 안 나온다.

### 17d. 아이템 폼 위쪽 좌우 2단 · 라벨 위치 글씨 (2026-09-17)
- 아이템 수정 폼 위쪽을 `.itf-2col`(grid 1fr 1fr)로 감쌌다: 왼쪽 `.itf-l` = 브랜드·아이템명·품번·시즌·컬러 설정·사이즈 설정 / 오른쪽 `.itf-r` = 가공비 및 담당 공장 연결(공임·수량별/리오더 공임·워싱·자수·나염·완성·검품).
- 작업경로부터 아래(원단·부자재)는 예전처럼 가로 전체. 1000px 미만은 한 줄씩(grid 1fr) — 폰은 기존과 동일.
- 감싸는 지점 3곳: 브랜드 `.fg` 앞(열기) · 「가공비 및 담당 공장 연결」 sec 앞(열 전환) · 「작업경로」 sec 앞(닫기).
- 작지 라벨 위치 입력(`.tp-lblpos input`): 13px·굵게·빨강(#c0392b)로 눈에 띄게.

### 17e. 개선사항 시트 — 사진별 코멘트 · 사진칸 고정 (2026-09-17)
- 사진 한 칸 = `.tp-qc-cell` (사진 + 그 밑 코멘트 한 줄). 코멘트 저장 자리 `qc.cmt.{슬롯}` (qcFront·qcBack·qcLining·qcDetail), 헬퍼 `_qcSlot`.
- 사진칸은 더 이상 남는 공간만큼 늘어나지 않음: `.tp-qc-grid{flex:0 0 auto}` + `.tp-qc-cell .tp-smp-box{flex:0 0 auto;height:210px}`(가로형 170px). 세로 비율을 낮춰 A4 한 장에 사진 4장 + 코멘트 + 개선사항이 같이 들어감.
- 인쇄: 사진 6.1cm(가로형 4.3cm), 개선사항 메모 2.6cm.

### 17f. 작지 실 「30수」 오표시 제거 · 부자재 가공비 줄 = 원단 줄 모양 (2026-09-17)
- 작지 원·부자재 표(`tpMaterialTableHTML`)가 실 규격이 비면 미니멈(threadMin) 값에 「수」를 붙여 「30수」로 보여주던 줄 삭제 — 미니멈 장수는 실 굵기가 아님. 규격은 size 칸만.
- 부자재 가공비 줄(`trimCostRowHTML` 일반 경로)도 `.tcf-row` 세 덩어리: 누가·무슨 가공(이름·종류·가공하는 집) / 가공비(금액·개당·컬러별·한번에·조건부 버튼·「옷 1장에 N개」·가공집 최소·원가 칩·조건부 요약·컬러 적용). data-f 칸 이름은 그대로라 colTrimCosts·계산 불변. 원단 전용 계산 문장(`.tcf-cost-say`)은 부자재 줄에 안 붙임.

### 17g·17h. 단가장 「계산 방식」·미니멈 두 칸·단위 추가 / 개선사항 시트 다듬기 (2026-09-17)
- 단가장 부자재 표 새 열 3개(규격 뒤): **계산 방식**(=발주 방식: 낱개·절·실, `setPbCalcMode`) · **미니멈 분배**(실 threadMin / 절·롤 minSplitQty+useMinSplit) · **미니멈 발주**(minOrderQty+hasMinOrder, `pbSetMin`). 지퍼·단추·스냅·라벨·바이어스·단추고리는 글자로만. 옛 롤·레이스 품목은 그 값일 때만 목록에 보임.
- 낱개 = 단위가 Y·절·R(야드류)면 orderType yard(절당 칸 비움), 아니면 count. 절 = roll+rollUnit 절. 실 = thread. 바꾸면 단위도 맞춤(`_pbCalcUnit`), 이 품목을 쓰는 아이템 줄이 있으면 confirm으로 같이 바꿈(옛값 `t._calcOld`). 어긋난 품목은 「단위와 다름」「줄 N 다름」 빨간 배지(`_pbTrimIssues`, 아이템 줄 인덱스 `_pbRowIdx` 0.8초 캐시).
- 단위: 「절」 추가(PB_UNITS) + 사용자 단위(`S.refData.trimUnits`, ＋ 새 단위…/단위 정리…). 사용자 단위는 계산 방식 안 건드림. 절 품목은 단위 절로 표시.
- **개당 단가 = 계산 단위 1개의 값**(절=1절 값, 야드=1y 값, 실=1콘 값, 개수=1개 값) — 칸 밑 회색 라벨(`_pbEachLbl`). **묶음단가는 돈 계산에 안 씀(참고)**: 야드 낱개는 묶음×1y=1절 값, 절은 1y 환산(1절÷묶음). 묶음 = 1절·1롤당 양. 머리글 title에 설명.
- 카드 옵션: 단가장 등록 부자재는 「미니멈」「미니멈 분배」 칩을 빼고 「미니멈 (단가장) 분배 N장 · 발주 N콘」만 표시. 자동 불러오기는 단가장 미니멈이 있으면 덮어씀(기준=단가장).
- 개선사항 시트: 사진 위 라벨·점선·둥근 모서리 없음, 사진끼리 붙임(1px 선), 사진 300px(가로형 230px, 인쇄 8.6/5.6cm), 사진 코멘트 3줄 textarea(`qc.cmt.*`, `.tp-qc-cm3`), 개선사항 칸 60px(¼).

### 17j. 작지 「라벨 위치」 글씨 안 먹던 것 고침 (2026-09-17)
- 증상: 17c에서 넣은 13px·굵게·빨강이 화면에서 11px·검정 보통으로 보임.
...
### 17l. 단가장 주소 고쳐도 「주소 중복」 빨간 글씨·헤더 뱃지가 안 바뀌던 것 (2026-09-17)
- 원인: 정보 패널·표 띠의 동/층/호수 onchange 가 `savePB()` 만 하고 표를 다시 안 그림 → 경고·뱃지는 렌더 때 계산되는 값이라 옛것이 남음.
- 고침: 동/층/호수 저장 뒤 `pbSoftRender()`(입력 중엔 미루고 표 밖으로 나가면 다시 그림). 정보 패널 열림 상태를 `window._bookInfoOpen[거래처]` 로 기억해 다시 그려도 열린 채.
...
### 17o. 발주서 부자재 로스 기본 0% 고정 (2026-09-17)
...
### 17r. 발주서 「염색」「스와치 전달」 묶음 앞 빈 줄 (2026-09-17)
- 이름·단가 바로 밑에 묶음이 오면 빈 줄을 안 넣는다(앞에 이미 출력된 컬러 줄이 있을 때만 빈 줄로 구분). 원단(_bln)·부자재(_tln) 둘 다.
...
### 18a. 부자재 카드 「단가 기준」 축이 컬러 체크하면 사이즈별로 튀던 것 (2026-09-18)
...
### 18d. 단가장 부자재 표 두 줄 카드 + 규격 전파 + 「줄 N 다름 · 맞추기」 (2026-09-18, 미리보기 2안 승인)
- 윗줄 12열(버튼·유형·품명·규격·계산 방식·단위·묶음·개당 단가·미니멈 분배·미니멈 발주·갱신·버튼), 아랫줄 `tr.pbt-r2`(무게·온즈·오무데·묶음단가·세트). 칸 밑 글자(`.pbt-sub2`) → 같은 줄 접미(`.pbt-suf`). 갱신은 올해면 「09. 18.」(title 에 전체). colspan 17→12. 세트 줄은 colspan 으로 개당 단가 열 아래 정렬. css v=20260918a.
- `pbSetSpecPart`: 단가장 규격 바꾸면 그 자재를 쓰는 아이템 줄 `size` 도 같이(등록 자재 규격 = 단가장 기준). 발주서 머리줄은 이름에 규격이 이미 있으면 안 붙임.
- 「줄 N 다름」 배지 = 이 자재를 쓰는 아이템 줄 중 계산 방식이 단가장과 다른 것. 버튼으로 바꿔 `pbSyncCalcRows` — 아이템 이름·현재 방식 목록 confirm 뒤 단가장 방식으로 맞춤.

### 18e. 발주서 카드 순서 = 아이템 순서 · 바이어스 원단출처 경고 · 초안 안전장치 (2026-09-18)
- 카드 순서: 원단처 → 부자재 줄 순(메인라벨은 t.supplier=브랜드) → 그 외(가공집 등)는 원래 순서. 예전엔 `Object.keys(o.suppliers)` 생성 순.
- 바이어스 카드: `_biasSrcWarn(t)` — 원단출처가 이 아이템 원단 이름·원단처와 안 맞으면 빨간 안내(신고: 다른 원단이 붙어 있음. 코드상 원단출처를 쓰는 곳은 카드 저장(colTR)뿐 — 원인 미확정).
- `loadDraft`: 기존 아이템 수정 중엔 거부(초안은 새 아이템 전용). css v=20260918b.

### 18f. 거래처 확인 「발주안함」 키에 용도 추가 (2026-09-18)
- 키 `거래처|자재명|색상`이라 같은 원단(엠마체크 보라)의 몸판·스트링감 줄이 같이 체크됨 → `|용도` 추가. 옛 키는 읽기 호환(`_matSkipKeyOld`), 옛 키가 켜진 채 한 줄만 끄면 나머지 용도 줄에 새 키로 옮김. 이름 변경 전파도 옛/새 키 둘 다 옮김.

### 18g. 바이어스 원단출처 어긋난 아이템 하루 한 번 알림 (2026-09-18)
- 원인 확정: 8/25~27 사이 버그(2f2a89f 에서 고침) — 바이어스재단 단가장 항목 하나를 모든 아이템이 공유해 autofill/잠금이 규격·원단출처를 다른 아이템 값으로 덮음. 그 사이에 열었던 아이템은 데이터가 이미 바뀐 채 남음(발송한 발주서는 스냅샷이라 정상). `_scanBiasSrc/_scanBiasSrcOnce` 가 켤 때 훑어 토스트로 아이템 이름 알림(하루 1회, `fpm_biasScanDay`). 카드 경고(18e)와 함께 손으로 고치면 됨.

### 18h. 발주안함 자재 = 발주서 본문 줄긋기·사선, 보내는 글에서 제외 / 바이어스 「발주서 값으로 되돌리기」 (2026-09-18)
...
### 18k. 세로 지시서 스와치 줄·지시사항 칸 조금 더 줄임 (2026-09-18)
- 줄 높이 `min(80,max(52,250/색수))`(4색 62px), 지시사항 min-height 30mm.

### 18l. 품번 중복 (2026-09-18)
- 생기는 길: ① 복제(`dupItem`)가 품번까지 복사 → 이제 품번 비움 ② 수동 저장 확인창에서 계속 ③ 자동저장은 조용히 저장(08-07 결정). 목록에 `.nc-dup` 「품번 중복」 배지(`_dupCodes`). css v=20260918d.

### 18m. 단가장 아랫줄 열 정렬 (2026-09-18)
- 아랫줄을 `td` 로 나눠 윗줄 열 밑에 맞춤: 무게=규격, 온즈=계산 방식, 오무데=단위+묶음, 묶음단가=개당 단가, 세트=미니멈 분배+발주. 라벨은 칸 위에 작게, 입력칸은 칸 폭 100%. css v=20260918e.

### 18n. 단가장 새 품목 고정·이름 바꾸는 중 이동 방지 / 부자재 카드 단가 기준 축 세션만 (2026-09-18)
- 「+ 품목」 새 줄(`_pbNewRow`)은 유형 맨 위 고정, 이름을 바꿔도 키 따라 유지, 표 밖으로 나가 다시 그릴 때 해제. `_pbRekeyRow` 는 윗줄·아랫줄·세트 줄 전부 새 키로.
- 18a 의 `t.priceAx` 저장 취소 — 세션(`_trimUI.ax`)만. 열 때 기본 = 사이즈 단가표 있으면 「둘 다」, 아니면 「컬러별」(컬러 칸 항상 보임). 「컬러당 발주」= 적용컬러 컬러 체크.

### 18o. 작지 치수표 글씨 10.5→12px (2026-09-18)
- `table.tp-spec td/th` 와 `.tp-cell/input` 둘 다.

### 18p. 발주서 품명 줄에 「> 출고처 이름」 (2026-09-18)
- `genPoText` 품명 줄 끝에 출고처(납품공장) 이름을 「> 엄지」로. 맨 밑 [출고처] 주소 블록은 그대로. 출고처 표기 옵션(includeAddr/includeFcName)이 켜진 카드만.

### 18q. 작지 이미지 저장 — 사진 비율 고정을 복제본 기준 한 벌로 (2026-09-18)
- 신고: 같은 시트를 두 번 저장했는데 한 번은 도식화가 옆으로 늘어남(09-07·09-17 에도 같은 계열).
- 원인: 비율 고정 두 벌(`_tpCloneFitImages` + 17k 블록)이 **살아있는 문서의 img 와 순서 번호로 짝지어** 크기를 읽음 → 저장 순간 화면이 다시 그려지거나 img 개수가 어긋나면 엉뚱한 사진과 짝 → 고정 누락 → html2canvas 가 object-fit 을 몰라 칸에 꽉 채워 그림. 타이밍 따라 됐다 안 됐다.
- 고침: 17k 블록 삭제, `_tpCloneFitImages` 를 복제본 img 자기 칸 크기·계산 스타일·자연 크기로 계산(자연 크기 없으면 같은 src 의 살아있는 img → `tpWaitImages` 가 채우는 `window._tpImgDim` 캐시). contain·scale-down·cover(부모가 자를 때만). 시험: 살아있는 문서에 img 를 끼워 순서를 어긋나게 해도 비율 유지 확인.

### 18r. 옛 버전 창은 다시 열기 전까지 클라우드에 안 올림 (2026-09-18)
- 신고: 메인라벨 포함 항목을 바꿔 저장해도 자꾸 옛값 — 10번 만에 저장됨. 로컬 재현(체크→저장→다시 열기)은 정상 → 동기화 쪽.
- `fpfAutoUpdate` 가 옛 버전을 감지했는데 편집 중·이미 한 번 시도라 못 다시 열 때 **띠만 띄우고 옛 코드로 계속 푸시**했음. 창이 둘(메인 + 단가장 새 창)이고 하루 배포가 많으면 버전이 갈려, 옛 창이 아이템을 _mt 새 도장으로 올려 다른 창의 저장을 통째로 덮음(아이템은 _mt 큰 쪽 통째 채택).
- 고침: 그 경우에도 `_appStaleBlock=true`(부팅 게이트와 같은 원칙). 로컬 저장은 그대로, 새로고침하면 올라감.


### 2026-09-21a — 가공집 최소 칸(정액 방식에도) · 원단 가공 담당 집 발주 카드 · 스와치 올리기 보강
- **가공집 최소**: 원단·부자재 가공비 카드에서 `컬러별`·`한번에`(정액)일 때도 「가공집 최소」 칸이 보인다. 정액이라 금액은 안 바뀌고 발주서 가공 줄에 「최소 ○y/개」로만 나간다(힌트 문구로 안내).
- **원단 가공 담당 집 카드**: 원단 가공비(tc.fabId)의 「가공하는 집」이 원단처와 다르면 담당 집 발주서에 `type:'proc'` 카드(`procUnit:'y'`)가 생긴다. 야드 = 요척 비움이면 그 원단 컬러 줄 발주 야드, 요척 적으면 장수×요척(+컬러마다 한 번 더 시킨 야드). 돈 0(통보용). 「옷 1장마다 더 시키기」 줄은 예전 재단 발주(바이어스재단) 길 그대로, 「컬러마다 한 번」은 이 카드(전체 야드)로. 담당 집 = 원단처면 예전대로 원단 줄 밑 가공 줄.
- 발주서 가공 줄: 이름에 종류가 이미 들어 있으면 종류 생략(「28mm 바이어스 재단 바이어스 재단」 방지).
- **스와치 올리기(mtAddPhotos 한 길)**: 클라우드 올리기 20초 제한 → 넘으면 이 기기 저장 + 자동 재업로드 대상(`_collectInlineImages` 에 단가장·직접추가 소재 스와치 포함). 브라우저가 못 읽는 사진(HEIC 등)은 그 장만 건너뛰고 안내. 소재 키는 `_encQ`(' → %27) — `'` 들어간 이름에서 onclick 이 깨지던 것. 원단 라이브러리 스와치도 같은 길.

### 2026-09-21b — 스와치 사진이 「추가됐어요」만 뜨고 안 붙던 원인
- 로그인 상태에선 저장(cloudSyncSave)·수신(applyRemoteShared)·아이템 자동저장(saveToPB) 때마다 `S.priceBook` 의 거래처·품목 객체가 **새 사본으로 바뀐다**. `mtAddPhotos` 가 올리기 시작할 때 잡은 항목(r)에 몇 초 뒤 사진을 붙이면 버려진 옛 사본에 붙어 화면·저장 어디에도 안 남았다.
- 규칙: **비동기(업로드·AI 등) 뒤에 단가장·소재·아이템에 값을 쓸 땐, 쓰는 순간 키로 다시 찾는다**(`_mtRec(key)` / `S.items[id]`). 객체 참조를 await 너머로 들고 가지 않는다. 자동 재업로드(`_collectInlineImages` 의 apply)도 같은 규칙.

### 2026-09-21c — 라벨 전용 발주서에도 「> 출고처」
- 메인라벨처·케어라벨처처럼 라벨만 있는 발주서는 「품명」 줄이 없다 → 라벨 블록 제목 줄(`*브랜드 / 품명 (날짜)` · `품명 (날짜)`) 끝에 ` > 출고처 이름`. 출고처 = 카드 출고처, 없으면 그 아이템 봉제공장. 조건은 품명 줄과 같다(「출고처 주소까지 포함」·「출고처 이름만」 중 하나가 켜져 있을 때). 원단처·부자재처는 09-18p 품명 줄 그대로.

### 2026-09-21d — 지퍼 슬라이더 「선택 안 함」 · 발주서 출력 줄
- 지퍼 카드 디테일의 슬라이더 상자 머리에 체크 「선택 안 함」(`t.noSlider`). 켜면 이름·컬러·단가를 비우고 상자를 회색·흐리게(`.tcd-sloff`), 끄면 방금 값(세션 `_trimUI[id].slKeep`) → 없으면 단가장 값으로 되돌림. 잠금 끌어오기(`_lockTrimPbFields`)는 켜져 있는 동안 슬라이더 칸을 건너뛴다.
- 단가장 저장(saveToPB): 선택 안 함인 아이템이 단가장에 적힌 슬라이더를 지우지 않는다. 단가장에도 슬라이더가 없으면 그 지퍼에 `noSlider` 표시 → 다음에 그 지퍼를 고르면 자동 체크.
- 「발주서 출력」 줄은 실제 발주서 지퍼 줄과 같은 규칙(`_zipPrevTxt`): 빈 칸을 「?」로 찍지 않고, 슬라이더가 없으면 「슬라이더 줄 없음」.

### 2026-09-21e·f — 지퍼 발주서 출력 문구 · 메인라벨 브랜드 자동
- 21e: 지퍼 카드 「발주서 출력」 줄 — 슬라이더가 없으면 아무 말도 안 붙임(「슬라이더 줄 없음」 문구 삭제). 슬라이더 선택 칸을 디테일 옆 큰 버튼으로 옮기는 시안(A·B·C)은 승인 대기.
- 21f: 메인라벨 카드 「브랜드(발주처)」가 비어 있으면 아이템 브랜드(`#f-brand`)로 자동 채움(`mainLabelCardNew` 첫머리) → 라벨명·포함 항목을 바로 고를 수 있다. 아이템 브랜드를 바꾸면(`pickItemBrand`) 브랜드가 빈 메인라벨 카드만 따라온다. 이미 다른 브랜드를 고른 카드는 그대로.

### 2026-09-21g — 지퍼 「슬라이더 선택 안 함」 큰 체크 칸 (승인 B안)
- 디테일 제목 옆(2단 화면) / 디테일 첫머리(좁은 화면)에 큰 체크 칸 `.tcd-noslbig`(36px·2px ink·그림자, 켜면 민트 #A5E6BA). 슬라이더 상자 안 작은 체크는 없앰. 제목과 한 줄에 서도록 지퍼 디테일 칸만 `.tcd-zipd::before{display:inline-block}`.
- 켜면 슬라이더 상자와 **로고 인쇄 상자 둘 다 회색**(`.tcd-sloff`) + 로고 인쇄 체크도 꺼짐(발주서에 로고 줄 없음·원가에 각인 단가 없음). 끄면 슬라이더 값·로고 체크 되돌림(세션 `_trimUI[id].slKeep`). 단가장의 슬라이더·로고 표시는 안 건드림.

### 2026-09-21h — 로스: 「기본 5%」 프리셋 삭제 · 새 발주서 원단도 0%
- 프리셋 「기본 5%」(%·5·전체)는 기본 목록에서 빼고, 저장된 목록에서도 기기마다 한 번 지운다(`fpm_lp_no5`). 새 기기 기본 목록 = 기본 0% · 라벨 +10개 · 단추 1벌치.
- 새 발주서 로스 기본값(`_ensureOrderLossRaw`): 원단·부자재 전부 0%(카드 로스 안 따라옴) · 라벨만 +개(qtyBuffer). 이미 값이 있는 발주서는 그대로. 단추 자동 여분은 사용자 결정 대기.

### 2026-09-21i — 가공비 「컬러별」 적용 기준에 「선택한 컬러」
- 적용 기준 칩 3개: 염색 켠 색만 / 전체 컬러 / **선택한 컬러**(청록 #0f766e). 선택한 컬러를 누르면 지금 적용 중인 컬러가 체크된 채로 컬러 체크칸이 나온다(한 컬러는 남겨야 함).
- 저장 자리는 새로 안 만듦: `colorBasis='all'` + `tc.colorLinks`(고른 컬러). 계산(원가·결제·원장·발주서)은 예전부터 이 목록을 읽어 그대로. 「전체 컬러」를 누르면 목록을 비운다.

### 2026-09-22a — 메인 스케줄표 · 리오더 스케줄표 (새 탭 2개)
- 왼쪽 메뉴 「생산」 묶음 맨 위: 메인 스케줄표(`msched`) · 리오더 스케줄표(`rsched`). 한 함수 `renderPSched('main'|'re')`, 부품 클래스는 `.psc .psc-*`(샘플 스케줄표와 같은 모양: 제목줄 · 안내/묶기 줄 · 숫자 상자 필터 · 브랜드(공장) 카드 + 표).
- 칸: 아이템(오더 날짜·N차·장수·빼기) · 공장 · 지금 단계 · 원자재/부자재(거래처마다 `[발주완료|발주 전] 거래처 · 입고 예정 · 입고 체크`) · 패턴·지시서(메인만, 갈 날 + 들어감) · 재단/출고(예정일 + D-day + 끝남 체크) · 판정.
- 날짜 칸 = 「9/24」 글자 + 숨긴 date 입력(누르면 `showPicker`). 작은 밑줄형(`.psc-d`)과 노란 상자형(`.psc-d.big`).
- **저장 자리**: 새로 만든 건 `o._prep.patDate/patDone`(패턴) · `o._prep.schOff[itemId]`(빼기)뿐. 나머지는 생산 대시보드와 같은 자리·같은 저장 함수(`pdSaveSupField`·`_pdSetSupArrived`·`pdSavePrep(Done)`·`pdSaveField`·`_pdSetFcDone`). 발주 칩은 오더 관리 발송완료를 보기만 하고 누르면 그 오더 발주서로.
- 판정(저장 안 함): 발주 안 된 곳 N → 자재 늦음(입고 예정 > 재단 예정) → 재단 준비 끝 → 진행 중 / 재단 완료 / 출고완료. 출고 끝 체크·출고완료 오더는 「끝난 것 N건 보기」로 접힘. 메인/리오더 = `getItemReorderNum().num` 1 / 2+.
- 보내는 글: 브랜드별 묶기 = 브랜드용(거래처 이름 없이 원단·부자재 입고, 재단·출고) · 공장별 묶기 = 공장용(거래처별 입고일, 패턴·지시서 전달, 재단·출고).
- 생산 대시보드 「원·부자재 입고」 줄에 **패턴** 카드 추가(지시서 앞, 같은 자리).

### 2026-09-22b — 「(추가발주)」 카드가 있으면 발주서 재계산이 영영 취소되던 버그
- `_refreshOrderSuppliersRaw` 안전장치(거래처 수가 줄면 교체 취소)가 「○○ (추가발주)」 카드를 세는 바람에, 그 카드가 하나라도 있는 오더는 재계산이 매번 취소됐다 → 미발송 카드에 옛 컬러 줄이 남음(「#그레이 - 0롤 (필요수량 0y)」), 발송취소해도 추가발주 카드가 원래 카드로 안 합쳐짐.
- 규칙: 추가발주 카드는 안전장치에서 세지 않는다. 원래 카드가 굳어 있으면(발송·출고·입고) 추가발주 카드 = 최신 계산 중 원래 카드에 없는 줄. 원래 카드가 미발송이면(발송취소 포함) 추가발주 카드를 없애고 원래 카드로 합친다(적어 둔 실제 발주·협상가·발주안함은 옮김). 보낸 추가발주 카드는 그대로.
- 굳은 카드 기준을 발송·출고·입고로 통일(도미노·동기화와 같은 기준).

### 2026-09-22c·d — 스케줄표: 메뉴 순서 · 급한 순서(끌어서) · 요일
- 22c: 왼쪽 메뉴 「생산」 = 생산 대시보드 → 오더 관리 → 메인 스케줄표 → 리오더 스케줄표 → 요척·불량·잔량.
- 22d: 줄 맨 앞 손잡이 `⠿`(`.psc-grip`)를 끌어 같은 묶음 안에서 순서를 바꾼다(끄는 줄 흐림 · 놓을 자리 파란 선). 놓으면 그 묶음 줄 전체에 1,2,3… 순번 저장(`o._prep.schPri[itemId]`), 순번 있는 줄이 먼저·없는 줄은 오더 날짜순. 검은 순번 배지(`.psc-pri`). 숫자 상자로 거르는 중엔 손잡이 숨김. 「급한 순서 지우기」로 되돌림. 보내는 글도 같은 순서.
- 22d: 날짜 옆 요일(`9/25금`, 토=파랑 · 일=빨강) — `_pschMDWHtml`.

### 2026-09-22e~h — 스케줄표 빼기 안내 · 선택 네모 제거 · 샘플 코멘트 표/확인서
- 22e·f: 스케줄표 「빼기」를 누르면 맨 아래 「뺀 것」을 펼치고 토스트로 알림. 줄 앞 선택 네모(여러 건 빼기)는 없앰 — 손잡이 ⠿ + 순번만.
- 22g: 샘플 코멘트 사이즈 표 조작 칸(td.sc-ctl)이 도구 줄 클래스(.sc-ctl{display:flex})와 이름이 겹쳐 손잡이와 ×가 어긋났다 → 표 칸은 `display:table-cell`, ×는 제 폭. **새 클래스는 기존 이름과 겹치지 않게**(이번 같은 충돌은 조용히 깨진다).
- 22h: 샘플 코멘트 확인서 — 빈 사진 칸을 안 만든다(이미 올린 사진이 있는 회차만 그 사진 칸 표시). 코멘트 칸 최소 높이 70mm, 남는 높이는 코멘트가 씀.

### 2026-09-22i — 공장 메모 · 코멘트
- 공장 수정 창에 「메모 · 코멘트」(`fc.memo`, 여러 줄). 공장 목록 카드에 노란 메모 띠(`.fc-memo-v`)로 같이 보여 비교. 목록 검색에도 메모가 걸린다.
- `saveFc` 는 이제 기존 공장 객체 위에 덮어쓴다(`Object.assign`) — 창에 없는 값이 저장할 때 사라지지 않게.

### 2026-09-22j — 작지: 지시서 치수표가 비었을 때 회차 치수 가져오기
- 그레이딩·안사양·완성 시트는 지시서 치수표(`sew.spec`)를 같이 쓴다. 회차(샘플·패턴) 치수는 09-08 결정으로 따로라, 회차에만 적었으면 이 시트들은 빈 표로 보인다(자동 합치기는 09-08f 에 중단 — 되살리지 말 것).
- 지시서 치수표에 값이 하나도 없고 회차에 값이 있으면 치수 제목 밑에 노란 안내 줄(`.tp-spechint`, 인쇄·이미지 저장에선 숨김) + 「샘플·패턴 N차 치수 그대로 가져오기」 버튼(`tpSpecAdoptRound`): 부위·전 사이즈의 패턴·완성·편차를 복사. 회차 쪽 값은 그대로. 지시서에 값이 이미 있으면 안 덮고 「불러오기」로 안내.

### 2026-09-22k — 작지 치수표: 편차 칸 좁게 · 숫자 칸 넓게 · 소수점 잘림
- 편차 th 38→26px, 치수표 칸 좌우 여백 4→1px(칸)·2→1px(입력), 자간 -0.2px → 사이즈 2개일 때 숫자 칸 약 36→41px(「101.8」 12px 글씨로 다 보임).
- 그래도 넘치는 칸(사이즈 3개 이상)은 `tpSpecFitCells` 가 그 칸 글씨만 0.5px 씩 줄인다(최소 8.5px, 줄인 칸끼리는 같은 크기). 값은 안 바뀜. 입력 중에도 그 칸만 다시 맞춤.

### 2026-09-22l — 작지 지시사항 글씨 +2
- `textarea.tp-instr`: 지시서·안사양 14→16px, 샘플·패턴/그레이딩(패턴 지시사항) 13→15px. 굵기·줄간격 그대로.

### 2026-09-22m — 샘플 코멘트 사이즈 표: 출처 = 같은 번호의 작지 회차
- 코멘트 N차의 패턴·지시 값은 작지 「샘플·패턴 N차」에 값이 있으면 거기서 온다(`_scPairSpec`). 없을 때만 예전처럼 값이 가장 많은 시트. 직접 고른 출처(`rr.sizeSrc`)는 그대로 우선.
- 표를 처음 저장할 때 출처(`sizeSrc/sizeSz`)를 같이 적는다. 출처 기록 없이 저장된 옛 표는 제목에 「저장된 표」로만 표시.
- 저장된 표가 같은 번호 회차에서 온 게 아니면 제목 줄에 「샘플·패턴 N차 치수로」 버튼(청록) — 패턴·지시만 바꾸고 실측은 그대로(`scSizeTake`).

### 2026-09-22n — 작지 스와치: 배색·단추 좌우 배치
- 스와치 줄의 배색 칸과 단추 칸을 위아래 → **좌우**(배색 2 : 단추 1). 둘 다 있으면 묶음(`.tp-swsub.both`)을 1.6으로 넓혀 배색 칸이 좁아지지 않게. 슬롯(swB_·swBtn_)·사진은 그대로.

### 2026-09-22o — 재단 방향은 작업지시서에서만 (승인 시안)
- 아이템 원단 카드의 「재단」 칸 삭제. 값은 그대로 원단 자료(`f.cutDir`)에 있고, 작업지시서 원·부자재 줄에서 적는다.
- 원단 줄: 값이 없으면 이름 옆 「＋ 재단 방향」(`.bom-cutadd`, 편집 화면 전용) · 값이 있으면 노란 띠(`.bom-cut`)를 눌러 고침 → 입력칸 + 자주 쓰는 말 칩(`TP_CUT_PRESETS`) + 지우기(`.bom-cutedit`). Enter·바깥 클릭=저장, Esc=취소. 인쇄·이미지엔 띠만.
- 원단마다 값 하나 → 어느 시트에서 적어도 전 시트 원·부자재 줄에 같이 찍힌다. 아이템 폼 저장은 cutDir 을 폼 값으로 덮지 않고 지금 아이템 값을 따른다(`saveItemForm`).

### 2026-09-22p — 작지 지시서·안사양: 견본·워싱·실컬러 줄을 도식화 위로
- 왼쪽 단 순서 = 안내 띠 → **견본·워싱·실컬러** → 도식화 → 지시사항 → 스와치. 세로·가로판 둘 다, 안사양도 같은 순서(같은 자료 sew.sample/wash/threadColor).

### 2026-09-22q — 택배 시재 내역서: 금액을 칸에서 바로
- 금액 칸이 브라우저 prompt 대신 숫자 입력칸(`.tl-qin.cl-amt`). 적고 Enter·다른 곳 클릭이면 저장(`clSetAmt(id,val)`), 값이 같으면 다시 안 그림. 이미지 저장은 자료로 따로 그려서 영향 없음.

### 2026-09-22r — 결제 명세서(새 디자인 2종): 사이즈 나눔 · 결제 관리와 같은 규칙
- `ipBuildGroupReceipt`·`buildItemReceiptHTML`(ISSUED 디자인): 회차를 출고일·컬러·**사이즈**·비품 여부로 묶어 「사이즈」 칸에 적는다(free 는 빈칸). 소계·총합계 colspan 4.
- 공임 단가 = `laborRate(it,size,_laborCtx(o,it))` (사이즈별·수량별·리오더) — 예전엔 `it.laborCost` 하나로만 곱해 사이즈별 공임 아이템은 결제 관리와 금액이 달랐다. 잔량(미출고)·수선중은 `_shipExcl` 로 제외(결제 관리·옛 명세서와 같은 규칙, 예전엔 이 두 명세서만 포함하고 있었음).

### 2026-09-23a — 출고 장수 이동 기록 1단계: 상태판 · 이동 버튼 3개 · 결제/명세서 연동
- 회차(`sch.shipments`) = 공장에서 나온 것만. 이미 나온 옷을 옮기는 일은 `sch.moves` 에 한 저장(bid) 단위로 줄 추가 `{from,to,color,size,qty,date,memo,rpType,bgDc/fromDc,resp,respName,dedAmt}`. 칸 = n 정상 · bg B품 · rp 수선중 · wb 완불.
- 계산은 전부 한 곳: `shipBoard(o,fcId,itemId)`(상태판·검산·마이너스 칸) · `shipEffRecs(o,fcId,itemId)`(결제·명세서용 가상 회차 = 회차 − 잔량·수선중 + 이동 ±) · `_smNetDelta(m)`(출고합 `recomputeShipping`/`fcShippedQty`). 결제 관리 아이템 줄·명세서 3종(옛 거래명세서·`ipBuildGroupReceipt`·`buildItemReceiptHTML`)이 `shipEffRecs` 를 읽는다. `sewLaborBase` 는 음수 장수도 더한다. 이동이 없는 오더는 결과가 전과 같다(검산 문제 28).
- 완불 = 건마다 책임처. `resp:'fc'` 봉제공장 책임 → 봉제 공임 0 · `'us'` 우리 → 공임 지급 · 공장id 다른 집 → 공임 지급 + 그 집 앞 공제(`S.deductions` id `dmv_…`, 기본 금액 = 봉제 공임 + 원단값 `calcFabYards`, 칸에서 수정). 수선 보내기·완불은 불량 관리에 자동 줄(`S.defectRecs` id `dfm_…`, 이동 기록이 주인 · 묘비로 삭제).
- 자리 = 생산 대시보드 아이템 카드 → 「출고」 수량 모달(`pdOpenQtyModal`, role 출고): 위에 상태판 띠(`.sm-strip`) + 「이동」 버튼 3개(`.sm-btns`: B품→정상 · 수선 보내기(정상/B품에서) · 수선 복귀(정상·B품·완불 나눠 적기)) → 폼은 수량 표 자리에 열림(`#sm-form`, `smOpenForm/smSaveForm`). 지난 회차 아래 「이동 기록」 목록(`.sm-mvs`, 삭제만 · 수정은 지우고 다시).
- 안전장치: 있는 장수보다 많이 적으면 저장 막음 · 회차 수정/삭제·이동 삭제로 어느 칸이 마이너스 나면 되돌리고 막음. 옛 「수선중」 회차는 손대지 않고 rp 칸으로 읽음(모달에 안내). 새 회차의 「수선중」 체크는 사라지고(옛 회차 수정 때만 보임) 「추가로 만든 옷」 체크(`rec.extra`, 표시만·공임 100%)가 생김.
- 결제 관리 카드 출고일 줄에 이동 태그(수선 보냄·수선 복귀·B품 → 정상·완불·추가)가 붙고 순장수(−5장 등)로 적힘. 다른 집 책임 완불은 「완불(다른 집 책임 · 공임 지급) N장」 줄.
- 아직 안 한 것(다음 단계): 결제 완료 = 사진 저장 + 잠금 + 풀기 + 차액 줄(3단계) · 오더관리 요약·대시보드 라벨·브리핑(4단계) · 인라인 수량 입력 A안(모달 → 카드 안 패널).

### 2026-09-23c — 수량 입력 인라인 패널(레트로) · 이동 기록 수정 · 회차 메모 보존 · 명세서 사이즈 가로
- 「출고」 등 수량 버튼(`.p-qty`, `pdQtyToggle`)을 누르면 모달 대신 카드 공정 줄 아래 `.pdq-wrap#pdq-<oid>__<itemId>` 에 패널(`.pdq`, 레트로 토큰 하드코딩: ink #0B0A05 · paper #FFFCF7 · mint · amber · peach)이 열린다. 같은 버튼 다시 누르면 닫힘. 대시보드가 안 보이는 탭·접힌 카드에서는 예전처럼 창(`showGenericModal`, `.pdq.pdq-modal`).
- 패널 순서(출고): 상태판 타일 5(`.pdq-board`) → 검산 줄(`.pdq-check`) → 결제 띠(`.pdq-pay`, 장수·금액=sewLaborBase) → 종류 세그먼트(정상·B품·잔량, 숨은 체크 `pd-grid-bg/lv` 조종 `pdqKind`) + 「추가로 만든 옷」 + 이동 버튼 3개 → 날짜·메모 → 작은 수량 표(`.pdq-grid`, 96px 칸) + 안내·취소·저장 → 지난 회차+이동 기록 표(`.pdq-led`: 날짜·무슨 일·어디서→어디로·장수·컬러·결제 영향·수정/삭제, 줄 클릭=컬러×사이즈). 다른 공정은 상태판·이동 없이 표만.
- 이동 기록 「수정」(`smEditMove` → `smOpenForm(kind,bid)`): 그 기록을 뺀 상태판(`_smBoardFor`)으로 칸을 열고 값을 채움(`_smFillForm`), 저장하면 같은 bid 로 교체(불량·공제 줄 id 유지).
...
### 2026-09-23e — 결제 관리 명세서 인라인
- 아이템 결제 카드의 「명세서」 버튼(`ipOpenReceipt`)이 창 대신 카드 아래 `.ip-rcpt#ip-rcpt-<oid>__<itemId>` 에 명세서를 펼친다(`ipRenderReceiptInline`). 머리줄에 복사·카카오톡·이미지 저장·닫기(예전 창과 같은 4개), 같은 버튼 다시 누르면 닫힘, 누른 버튼은 `.on`. `renderItemPay` 를 감싸 다시 그려도 열린 명세서 유지. 카드가 안 보이면 예전 창(`openItemReceiptModal`). 통합 명세서(선택 여러 개)는 그대로 창.

### 2026-09-23f — 출고 패널 위치 정리: 적는 것은 위 상자 하나 · 기록 가운데 · 상태판 맨 밑
- 패널 순서 = 안내줄 → `.pdq-inbox`(테두리 2px 상자: 첫 줄 「적을 것」 세그먼트 `#pdq-what` = 출고 회차 · B품→정상 · 수선 보내기 · 수선 복귀 → 고른 폼만 상자 안에 표시. 출고 회차 = 종류 줄+날짜·메모+수량 표+저장, 이동 = `#sm-form` 이 상자 안에서 `pd-grid-main` 을 대신함) → 지난 회차·이동 기록 표 → 상태판·검산·결제 띠(맨 밑). 다른 스타일은 23e 그대로(사용자 「위치만 정리」).
- `smOpenForm` 이 `pd-grid-main` 을 숨기고 세그먼트를 그 폼으로 표시(`_pdqWhatMark`), `smCloseForm`(취소·저장 뒤)이 되돌림(`pdqWhat('round')`). 이동 버튼은 옷이 없으면 비활성.

### 2026-09-23g — 출고 패널: 회차 저장 뒤에도 열어 둠
- `pdGridSave` 가 인라인 패널이 열려 있던 경우 저장·다시 그린 뒤 `pdOpenQtyModal` 로 같은 자리에 새 상태(빈 표·갱신된 기록·상태판)로 다시 연다. 창(모달)일 때는 전처럼 닫힘.

### 2026-09-23h — 대시보드 카드·오더관리 출고 숫자도 상태판과 같게 (수선중·완불 제외)
- 생산 대시보드 아이템 카드의 「출고 N · 비품 N」·진행 막대·컬러×사이즈 매트릭스가 회차 합이 아니라 `shipBoard` 기준(정상 n + B품 bg)으로 바뀜. 수선중·완불이 있으면 「· 수선중 N · 완불 N」이 요약 줄에 붙음(`rpTotal/wbTotal`). 오더관리 공장 카드의 출고 수량(`_shipSum`)도 이동 순장수(`_smNetDelta`) 반영. 회차·이동이 없는 오더는 전과 같음.

### 2026-09-23i — 폰 하단탭 「더보기」 전체 메뉴: 웹 순서 · 묶음 제목 + 큰 줄
- `PD_NAVTABS` 를 웹 사이드바(`.ntab`) 순서와 똑같이 정렬(택배 송장 추가, `full`=전체 메뉴 이름·`label`=하단탭 짧은 이름). `PD_NAVGROUPS` = 사이드바 구분(아이템·생산·작업·결제·단가·기타).
- 시트(`btabSheet(true)`)는 92vh 까지 올라오고 위에 고정 머리(전체 메뉴 · 닫기 · 묶음 이동 칩 `btabJump`), 아래 묶음마다 제목 + 2열 큰 줄(`.bts-row`, 48px). 지금 탭은 색, 하단탭에 있는 것은 「하단」 표시. 「하단 탭 4개 바꾸기」는 그대로.

### 2026-09-24a — 아이폰에서 자동 파일 백업 건너뜀
- `_checkAutoFileBackup` 가 `_fpfIsIOS()` 이면 바로 돌아간다. 아이폰은 `_fpfDownload` 가 공유 시트를 띄워야 파일을 저장할 수 있어 하루 두 번 팝업이 떴다. 폰은 매시간 IndexedDB 백업 + 클라우드 동기화로 보호, 파일 백업은 컴퓨터가 담당. 설정의 「백업 저장」 버튼은 그대로(누르면 공유 시트).
