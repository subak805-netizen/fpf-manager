...
- 2026-06-09(3): **브랜드 탭에 라벨·완성부자재 참조이미지 → 작업지시서 자동 불러오기 (브랜드 로고 패턴 확장).** 브랜드 관리 카드에 `📎 라벨·완성부자재 사진` 4슬롯(메인라벨·케어라벨·완성부자재·단추/스냅, `renderBrandRefImagesSection`) 업로드 → `b.refImages[slot]`에 저장(`brandRefImgUpload`→Storage `brandrefs/{co}/{brandId}_{slot}_{ts}.jpg`, 폴백 dataURL, **URL만 JSON 저장**). 작업지시서 봉제탭 사진칸(`_iz`)이 **아이템 개별 이미지 없으면 브랜드 이미지로 폴백**(`tpBrandRefImg`) + `브랜드` 뱃지(삭제 버튼 없음, 클릭 시 아이템 개별 업로드로 오버라이드→×삭제로 폴백 복귀). 검증: 브랜드 3슬롯 설정→지시서 브랜드뱃지 표시, 아이템 오버라이드 시 뱃지 사라지고 ×표시, JSC SYNTAX OK.
