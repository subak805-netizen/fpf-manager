...
> **📌 최신 진행상황·규칙·다음 과제는 `HANDOVER.md`(2026-06-10)를 먼저 보세요.** 이 CLAUDE.md는 *누적 이력+구조 레퍼런스*라 오래된 섹션이 섞여 있어요. 새 작업 순서: ① HANDOVER.md(현재 상태) → ② 필요 시 이 문서·DESIGN_GUIDE.md에서 상세 검색.

> 새 채팅에서 이 문서를 붙여넣고 `index.html`(또는 프로젝트의 파일)을 올리면 바로 이어서 작업 가능.

> ## ⚠️ 커밋 충돌 방지 — 순서 반드시 지킬 것 (여러 채팅방 동시 작업 중)
> 사용자는 **여러 채팅방/기기에서 동시에** 이 저장소(단일 `index.html`)를 작업한다. 순서를 안 지키면 한 방의 작업이 통째로 덮여 사라진다. **모든 코드 작업은 아래 순서를 지킨다:**
> 1. **작업 전 `git pull`** (또는 `git fetch && git status -sb`로 최신 확인) — 남의 최신 커밋부터 받기.
> 2. **편집 → 문법검사(SYNTAX OK)**.
> 3. **끝나면 바로 `git add` → `commit` → `push`** (작업을 오래 미커밋 상태로 두지 말 것).
> 4. **push 직전 `git status -sb`** 로 origin 대비 ahead/behind 확인. behind면 `git pull` 먼저.
...
### G. 이어진 폼 정비 + 나브 (e9b79af 이후)
- 부자재 카드: 단추 레이아웃(부자재처+동+호수 / 부자재명+크기+스타일 / 색상+단가+로고버튼 / 요척+로스), 지퍼 재배치(부자재처+동호수 / 지퍼이름+호수+기장 / 방향+이빨컬러 / 슬라이더→단가 / 요척+로스), 바이어스 가공처+동호수, 단추/지퍼 **로고인쇄 버튼 크게**, 슬라이더 "슬라이더"라벨 삭제·모델명→슬라이더이름, 방향 select 각진(radius0)+높이통일.
- **자동호출 강화(autofillTrim)**: 부자재명을 *다른 이름으로 바꾸면* 단가·크기·폭·혼용률·절당yard·사이즈별단가표까지 새 값으로 **덮어씀**(요척/로스 등 직접입력값은 보존). `_ft._afName`으로 이름변경 감지. 크기칸=저장된 sizeRates 사이즈 datalist 드롭다운, 고르면 단가 자동매칭.
- **원단 컬러별 단가 복원**: fabRowHTML 적용컬러 줄에 컬러별 `단가` 입력 부활(data-fab-col-price→colorLinks[].price, colFR 수집), calcSups 원단 push에 컬러별 단가 반영(`_clPriceF`→unitPrice/colorPrice). 컬러 비우면 기본 야드단가.
